...
def main():
    ...
    analysis = http_json(f"{base_url}/api/admingeneration/reference/analyze", method="POST", body={
        ...
        "title": Path(video_file).name,
    })
    analysis_id = analysis.get("analysisId") or analysis.get("analysis", {}).get("id")
    print(f"✅ analysis created: {analysis_id}")

    job = http_json(
        f"{base_url}/api/admingeneration/reference/analyze/{analysis_id}/worker-jobs",
        method="POST",
        body={"requestedProfile": "admin_full"},
    )
    job_id = job["job"]["id"]
    ...
    editor = http_json(
        f"{base_url}/api/admingeneration/editor/from-analysis",
        method="POST",
