...
# Overwrite the corrupted backend route cleanly.
...
# Overwrite the corrupted client helper cleanly.
...
# Patch StreamsWorkspaceShell import + helper.
...
if "async function runRealSessionAction" not in shell:
    ...
    if marker not in shell:
        raise SystemExit("No blocked action function found in StreamsWorkspaceShell.jsx")
...
# Replace exact blocked calls where present.
for old, new in {
...
# Patch runtime return to expose session id aliases + refresh function.
...
if "refreshSidebarData," not in runtime:
    runtime = runtime.replace("    sessions,", "    sessions,\n    refreshSidebarData,", 1)
...
print("Repaired and built real chat session pin/archive/delete route + client wiring.")
