...
const configError = !UPSTREAM_URL || !isValidHttpUrl(UPSTREAM_URL)
...
// ── Turn execution ─────────────────────────────────────────────────────────
async function executeTurn(socket, session, turnId, message, context, log) {
    // Mark running
            ...
            switch (event) {
                ...
                case "done": {
                    if (data.ok === true) {
                        ...
                        hadError = true;
                        const errMsg = typeof data.error === "string" ? data.error : "Orchestrator reported failure";
...
async function handleMessage(socket, raw, session, log) {
    if (configError) {
        await send(socket, {
            type: "error",
            sessionId: session.sessionId,
    ...
    switch (msg.type) {
        ...
        case "session.turn": {
            const turnId = typeof msg.turnId === "string" && msg.turnId ? msg.turnId : (0, node_crypto_1.randomUUID)();
...
// ── HTTP server ────────────────────────────────────────────────────────────
function handleHttpRequest(req, res, log, cfgError) {
    const url = req.url ?? "/";
    if (url === HEALTH_PATH) {
...
function isValidHttpUrl(value) {
    try {
        const u = new node_url_1.URL(value);
