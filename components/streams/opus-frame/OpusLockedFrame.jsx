...
function findOutputUrl(data) {
  return String(
    data?.outputUrl ||
      ...
      data?.result?.result?.outputUrl ||
      data?.result?.result?.artifactUrl ||
      "",
  );
}
...
function findGenerationId(data) {
  return String(data?.generationId || data?.result?.generationId || data?.result?.result?.generationId || "");
}
...
export default function OpusLockedFrame() {
  ...
  const activeType = createTypes.find((item) => item.id === activeTypeId) || createTypes[0];
  ...
  const directorSetup = compileFinalDirectorSetup({
    fields,
    generationTypeId,
    visualCardId,
    visualCardTitle: selectedVisualCard?.title,
    provider,
    workflowMode,
    acceptedReferences,
  });
  ...
  function applyGenerationType(item) {
    setGenerationTypeId(item.id);
    setProvider(item.provider);
    setField("aspectRatio", item.aspectRatio);
    setWorkflowMode(item.id === "freestyle-draft" ? "freestyle-draft" : "structured-production");
    const firstCard = getVisualCardsForType(item.id)[0];
    if (firstCard) {
      setVisualCardId(firstCard.id);
      setFields((current) => ({ ...current, ...firstCard.fieldPreset }));
    }
    setSetupStage(item.id === "freestyle-draft" ? "director-setup" : "visual-library");
  }

  function applyVisualCard(card) {
    setVisualCardId(card.id);
    setFields((current) => ({ ...current, ...card.fieldPreset }));
    setSetupStage(card.referenceTypes.length ? "reference-intake" : "director-setup");
  }

  async function uploadReferenceFile(file, role = "uploaded-reference") {
    const form = new FormData();
    form.append("file", file);
    form.append("projectId", fields.projectId || DEFAULT_PROJECT_ID);
    ...
    const next = {
      type: data.asset?.assetType || role,
      role,
      assetId: data.asset?.assetId || null,
      url: data.asset?.sourceUrl || data.asset?.storage?.publicUrl || data.analysis?.sourceUrl || "",
      analysisId: data.analysisId || data.analysis?.id || null,
      accepted: true,
    };
    setAcceptedReferences((current) => [...current, next]);
    if (next.analysisId) setAnalysisId(next.analysisId);
    setStatus(`Accepted reference: ${role}.`);
    return next;
  }
  ...
  async function onReferenceFileChange(event) {
    ...
    if (!file) return;
    try {
      await uploadReferenceFile(file, selectedVisualCard?.referenceTypes?.[0] || "uploaded-reference");
      setSetupStage("director-setup");
    } catch (error) {
  ...
  async function startGuidedRecording() {
    try {
      const stream = await requestGuidedCaptureStream();
      streamRef.current = stream;
      const recorder = createGuidedRecorder(stream, async (blob) => {
        setIsRecording(false);
        try {
          const data = await uploadGuidedCapture({
            blob,
            projectId: fields.projectId || DEFAULT_PROJECT_ID,
            referenceType: recordingStep.referenceType,
          });
          const next = {
            type: "video",
            role: recordingStep.referenceType,
            assetId: data.asset?.assetId || null,
          ...
          setAcceptedReferences((current) => [...current, next]);
          if (next.analysisId) setAnalysisId(next.analysisId);
          setStatus(`Guided recording accepted: ${recordingStep.title}.`);
          setSetupStage("director-setup");
        } catch (error) {
          setStatus(error instanceof Error ? error.message : String(error));
        }
      ...
      recorder.start();
      setIsRecording(true);
      setStatus(`Recording: ${recordingStep.title}.`);
      setTimeout(() => {
  ...
  async function generateNow() {
    ...
    try {
      const setup = compileFinalDirectorSetup({
        fields,
        generationTypeId,
        visualCardId,
        visualCardTitle: selectedVisualCard?.title,
      ...
      if (workflowMode === "structured-production" && setup.missingRequiredItems.length) {
        setStatus(setup.missingRequiredItems[0]);
        setSetupStage("director-setup");
        return;
      }
      ...
      const response = await fetch("/api/admingeneration/routed-submit-v2", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          kind,
          provider: workflowMode === "freestyle-draft" ? "fal" : provider,
          projectId: fields.projectId,
          workspaceId: fields.projectId,
          prompt: workflowMode === "freestyle-draft" ? fields.mainPrompt : setup.providerReadyPrompt,
          aspectRatio: fields.aspectRatio === "N/A" ? undefined : fields.aspectRatio,
          duration: workflowMode === "freestyle-draft" ? "5s" : `${setup.durationPlan.requestedDurationSeconds}s`,
          longVideo: setup.durationPlan.requiresStitching,
          sourceImageUrl: anchor?.url || undefined,
          imageUrl: anchor?.url || undefined,
          storyBible: setup.storyBible,
          metadata: {
            source: "admingeneration-gap-finish-pass",
            workflowMode,
            generationTypeId,
            visualCardId,
            acceptedReferences,
            finalDirectorSetup: setup,
            durationPlan: setup.durationPlan,
            fields,
          },
        }),
      });
      ...
      setGenerationId(findGenerationId(data));
      const nextAnalysisId = findAnalysisId(data);
      if (nextAnalysisId) setAnalysisId(nextAnalysisId);
      setStage("result");
      setSetupStage("result");
      setStatus(response.ok ? "Generation request submitted through runtime." : "Generation blocked/error.");
  ...
  function openOutputEditor() {
    if (analysisId) {
      window.open(`/admingeneration/editor?analysisId=${encodeURIComponent(analysisId)}`, "_blank", "noopener,noreferrer");
      return;
    }
    if (generationId) {
      window.open(`/admingeneration/editor?generationId=${encodeURIComponent(generationId)}`, "_blank", "noopener,noreferrer");
      return;
    }
    setStatus("Output editor needs a real analysis ID or generation ID.");
  }

  function renderCenterSetup() {
    if (workflowMode === "freestyle-draft" && setupStage !== "generating" && setupStage !== "result") {
      return <FreestyleDraftPanel fields={fields} setField={setField} generateNow={generateNow} isGenerating={isGenerating} setWorkflowMode={setWorkflowMode} />;
    }
    if (setupStage === "type-selector") {
      return <TypeSelector selectedId={generationTypeId} onSelect={applyGenerationType} />;
    }
    if (setupStage === "visual-library") {
      return <VisualLibrary cards={visualCards} selectedId={visualCardId} onSelect={applyVisualCard} />;
    }
    if (setupStage === "reference-intake") {
      return (
        <ReferenceIntake
          selectedVisualCard={selectedVisualCard}
          ...
          startGuidedRecording={startGuidedRecording}
          stopGuidedRecording={stopGuidedRecording}
          previewVideoRef={previewVideoRef}
          continueToDirector={() => setSetupStage("director-setup")}
        />
      );
    }
    if (setupStage === "director-setup") {
      return <DirectorSetupPanel setup={directorSetup} fields={fields} setField={setField} generateNow={generateNow} isGenerating={isGenerating} />;
    }
    if (setupStage === "generating") {
      return <GeneratingPanel status={status} durationPlan={durationPlan} />;
    }
            ...
            <div className="movie-generate-state">
              <section className="compact-prompt-top">
                <div className="mode-row generation-modes">
                  {["structured-production", "freestyle-draft", "assistant"].map((item) => <button className={`mode ${workflowMode === item ? "active" : ""}`} key={item} onClick={() => item === "assistant" ? setHelperOpen(true) : setWorkflowMode(item)} type="button">{item === "structured-production" ? "✦ Structured Production" : item === "freestyle-draft" ? "⚡ Freestyle Draft" : "✨ AI Assistant"}</button>)}
                </div>
                <div className="prompt-and-cards"><label className="wide-prompt-field"><span>Describe what you want to create…</span><textarea value={fields.mainPrompt} onChange={(event) => setField("mainPrompt", event.target.value)} /><b>{fields.mainPrompt.length} / 2000</b></label><div className="prompt-chip-row"><button type="button" onClick={() => setSetupStage("type-selector")}>Type</button><button type="button" onClick={() => setSetupStage("visual-library")}>Visual Library</button><button type="button" onClick={() => setSetupStage("reference-intake")}>References</button><button type="button" onClick={() => setSetupStage("director-setup")}>Final Setup</button><button className="generate-cta" disabled={isGenerating} onClick={generateNow} type="button">{isGenerating ? "Submitting…" : "Generate ✨"}</button></div></div>
                <div className="create-type-strip compact">{createTypes.map((item) => <button className={`create-type-card ${item.id === activeTypeId ? "active" : ""}`} key={item.id} onClick={() => { setActiveTypeId(item.id); setProvider(item.provider); setField("aspectRatio", item.ratio); }} type="button"><span>{item.icon}</span><strong>{item.title}</strong><small>{item.kind}</small></button>)}</div>
              </section>

              <section className="movie-production-stage">
                <aside className="production-column left-prod"><ProductionCard title="1. Scene" fieldKey="scene" value={fields.scene} onChange={(value) => setField("scene", value)} textarea /><ProductionCard title="2. Subject" fieldKey="subject" value={fields.subject} onChange={(value) => setField("subject", value)} textarea /><ProductionCard title="3. Environment" fieldKey="environment" value={fields.environment} onChange={(value) => setField("environment", value)} textarea /><ProductionCard title="4. Emotional Intent" fieldKey="emotionalIntent" value={fields.emotionalIntent} onChange={(value) => setField("emotionalIntent", value)} textarea /><ProductionCard title="5. Mood" fieldKey="mood" value={fields.mood} onChange={(value) => setField("mood", value)} /></aside>
                <div className="center-preview-stack"><div className="preview-toolbar"><div><strong>{selectedGenerationType.title}</strong><span>{status}</span></div><div><button type="button" onClick={() => setSetupStage("type-selector")}>Type</button><button type="button" onClick={() => setSetupStage("visual-library")}>Library</button><button type="button" onClick={() => setSetupStage("reference-intake")}>Intake</button><button type="button" onClick={() => setSetupStage("director-setup")}>Setup</button></div></div><div className="large-center-preview">{renderCenterSetup()}</div><div id="streams-preview-analyzer-slot" className="preview-analyzer-slot" /><div className="mini-keyframes-row">{Array.from({ length: Math.min(durationPlan.requiredClipCount || 1, 8) }).map((_, index) => <button className={index === 0 ? "active" : ""} key={index} type="button"><span>{durationPlan.requiresStitching ? `clip ${index + 1}` : `${index}s`}</span></button>)}</div></div>
                <aside className="production-column right-prod"><ProductionGroup title="6. Camera" fields={fields} setField={setField} keys={["shotType", "cameraPosition", "cameraMovement", "lens", "depthOfField", "composition"]} /><ProductionGroup title="7. Lighting" fields={fields} setField={setField} keys={["primaryLighting", "accentLighting", "rimLight", "atmosphere"]} /><ProductionGroup title="8. Motion" fields={fields} setField={setField} keys={["characterMotion", "environmentMotion", "motionQuality"]} /><ProductionGroup title="9. Style" fields={fields} setField={setField} keys={["visualStyle", "filmReference", "productionDesign", "humanRealism"]} /><ProductionGroup title="10. Script / Performance Guide" fields={fields} setField={setField} keys={["spokenIntent", "preScriptLine", "performanceBeat", "gestureDirection", "facialExpression", "lipSyncNeed", "durationTarget", "voiceoverUseLater"]} /></aside>
              </section>

              <section className="movie-output-band"><div className="negative-card"><strong>11. Negative Prompt / Restrictions</strong><textarea value={fields.negativePrompt} onChange={(event) => setField("negativePrompt", event.target.value)} /></div><div className="output-card"><strong>12. Output Settings</strong><div className="output-inline-fields"><label><span>Provider</span><select value={provider} onChange={(event) => setProvider(event.target.value)}><option value="openai">OpenAI</option><option value="fal">fal.ai</option><option value="runway">Runway</option><option value="kling">Kling</option><option value="veo">Veo</option><option value="elevenlabs">ElevenLabs</option></select></label><label><span>Duration</span><input value={fields.duration} onChange={(event) => setField("duration", event.target.value)} /></label><label><span>Aspect</span><input value={fields.aspectRatio} onChange={(event) => setField("aspectRatio", event.target.value)} /></label><label><span>Frame Rate</span><input value={fields.frameRate} onChange={(event) => setField("frameRate", event.target.value)} /></label><label><span>Quality</span><input value={fields.qualityGoal} onChange={(event) => setField("qualityGoal", event.target.value)} /></label><button disabled={isGenerating} onClick={generateNow} type="button">{isGenerating ? "Submitting…" : durationPlan.requiresStitching ? `Generate ${durationPlan.requiredClipCount}-Clip Video ✨` : "Generate Video ✨"}</button></div><small>{durationPlan.userMessage}</small></div></section>
            </div>
          ) : (
...
function GeneratingPanel({ status, durationPlan }) {
  return <div className="analysis-float-card" style={{ position: "relative", width: "min(760px, 96%)", margin: "0 auto" }}><strong>Generating</strong><p>{status}</p><p>{durationPlan.userMessage}</p><p>Runtime stages: submit → provider jobs → polling → durable artifact → editor handoff.</p></div>;
}

function PreviewSetup({ outputUrl, status, openOutputEditor }) {
  return <div className="analysis-float-card" style={{ position: "relative", width: "min(760px, 96%)", margin: "0 auto" }}><strong>{outputUrl ? "Output Ready" : "Output Pending"}</strong><p>{status}</p>{outputUrl ? <button type="button" onClick={() => window.open(outputUrl, "_blank", "noopener,noreferrer")}>Open Output</button> : null}<button className="primary" onClick={openOutputEditor} type="button">Open in Full Editor</button></div>;
}

function ProductionCard({ title, fieldKey, value, onChange, textarea = false }) {
  const options = getFieldOptions(fieldKey);
  return <label className="production-card"><span>{title}</span>{options.length ? <select value={value} onChange={(event) => onChange(event.target.value)}>{options.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}<option value={value}>{value}</option></select> : null}{textarea ? <textarea value={value} onChange={(event) => onChange(event.target.value)} /> : <input value={value} onChange={(event) => onChange(event.target.value)} />}</label>;
}

function ProductionGroup({ title, fields, setField, keys }) {
  return <section className="production-group-card"><strong>{title}</strong><div>{keys.map((key) => <ProductionCard key={key} title={key.replace(/([A-Z])/g, " $1")} fieldKey={key} value={fields[key] || ""} onChange={(value) => setField(key, value)} />)}</div></section>;
}
...
function ResultWorkspace({ fields, status, outputUrl, jobResult, setStage, generateNow, isGenerating, openOutputEditor }) {
  ...
  return <div className="result-state-grid tighter"><section className="result-main-card tighter"><div className="result-action-bar"><div><strong>{hasRealOutput ? "✅ Generated Video Ready" : "🟡 Generation Request Saved"}</strong><span>{fields.duration}s · {fields.aspectRatio} · {fields.frameRate} · {status}</span></div><div><button disabled={!hasRealOutput} type="button" onClick={() => outputUrl && window.open(outputUrl, "_blank", "noopener,noreferrer")}>Download</button><button disabled={isGenerating} onClick={generateNow} type="button">Regenerate</button><button disabled={isGenerating} onClick={generateNow} type="button">Create Variation</button><button onClick={() => setStage("generate")} type="button">Edit Prompt</button><button className="primary" onClick={openOutputEditor} type="button">Open in Full Editor</button></div></div><div className={`output-player ${hasRealOutput ? "ready" : "blocked"}`}>{hasRealOutput ? (outputUrl.match(/\.(mp4|webm|mov)(\?|$)/i) ? <video src={outputUrl} controls playsInline /> : <img src={outputUrl} alt="Generated output" />) : <div className="no-output-state compact"><strong>No real output file returned yet</strong><p>The job request is saved. This screen will not show a fake generated video.</p><code>{readable(jobResult) || "Awaiting provider output"}</code></div>}<div className="video-controls result-controls"><b>▶</b><span>0:00 / 0:{String(fields.duration || 8).padStart(2, "0")}</span><i /><em>CC</em><em>1x</em><em>⛶</em></div></div><div className="result-lower-grid tighter"><div className="output-specs"><strong>Output Specs</strong><p>Duration <span>{fields.duration}s</span></p><p>Format <span>{hasRealOutput ? "Provider output" : "Pending"}</span></p><p>Resolution <span>{fields.aspectRatio}</span></p><p>Quality <span>{fields.qualityGoal}</span></p></div><div className="iteration-tools"><strong>Iteration Tools</strong><div><button type="button" disabled={isGenerating} onClick={generateNow}>Create Variation</button><button type="button">Extend Video</button><button type="button">Reframe Shot</button><button type="button" onClick={() => setStage("generate")}>Edit with Prompt</button></div></div></div></section><aside className="result-summary-rail tighter"><SummaryCard title="Generation Summary" text={hasRealOutput ? "A generated output is available." : "Request saved. Real provider output has not returned yet."} /><SummaryCard title="Prompt Snapshot" text={fields.mainPrompt} /><SummaryCard title="Camera Settings Summary" text={`${fields.cameraMovement} · ${fields.shotType} · ${fields.lens}`} /></aside></div>;
}
