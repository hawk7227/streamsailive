...
import { createGuidedRecorder, requestGuidedCaptureStream, uploadGuidedCapture } from "@/lib/admingeneration/guided-capture-client";
...
const RAW_REFERENCE_BASE = "https://raw.githubusercontent.com/hawk7227/hipa-doctor-panel/master/public/reference-presets/video-generation/provider";

const REFERENCE_ASSET_URLS = {
  "clear-character-anchor": `${RAW_REFERENCE_BASE}/multi-image-consistency-front-01.jpg`,
  "face-close-up": `${RAW_REFERENCE_BASE}/multi-image-consistency-front-01.jpg`,
  "mid-shot-speaker": `${RAW_REFERENCE_BASE}/multi-image-consistency-front-01.jpg`,
  "full-body-action-pose": `${RAW_REFERENCE_BASE}/full-body-action-pose-02.jpg`,
  "product-object-hero": `${RAW_REFERENCE_BASE}/product-object-hero-01.jpg`,
  "establishing-environment": `${RAW_REFERENCE_BASE}/establishing-environment-02.jpg`,
  "lighting-mood-frame": `${RAW_REFERENCE_BASE}/lighting-mood-frame-01.jpg`,
  "composition-negative-space-frame": `${RAW_REFERENCE_BASE}/composition-negative-space-frame-01.jpg`,
  "multi-image-consistency-set": `${RAW_REFERENCE_BASE}/multi-image-consistency-front-01.jpg`,
  "tail-end-frame": `${RAW_REFERENCE_BASE}/tail-end-frame-01.jpg`,
  "mask-friendly-clean-source": `${RAW_REFERENCE_BASE}/mask-friendly-clean-source-01.jpg`,
  "context-workstation-use-case-frame": `${RAW_REFERENCE_BASE}/context-workstation-use-case-frame-01.jpg`,
};

const MULTI_IMAGE_REFERENCE_URLS = [
  `${RAW_REFERENCE_BASE}/multi-image-consistency-front-01.jpg`,
  `${RAW_REFERENCE_BASE}/multi-image-consistency-three-quarter-01.jpg`,
  `${RAW_REFERENCE_BASE}/multi-image-consistency-side-action-01.jpg`,
  `${RAW_REFERENCE_BASE}/multi-image-consistency-context-01.jpg`,
];

function presetUrlFor(type) {
  return REFERENCE_ASSET_URLS[type] || REFERENCE_ASSET_URLS["clear-character-anchor"];
}
...
const findOutputUrl = (data) => String(data?.outputUrl || data?.videoUrl || data?.assetUrl || data?.result?.outputUrl || data?.result?.result?.outputUrl || data?.result?.result?.artifactUrl || "");
const findAnalysisId = (data) => String(data?.analysisId || data?.analysis?.id || data?.result?.analysisId || data?.result?.analysis?.id || "");
const findGenerationId = (data) => String(data?.generationId || data?.result?.generationId || data?.result?.result?.generationId || "");
...
export default function OpusLockedFrame() {
  ...
  const [setupStage, setSetupStage] = useState("visual-library");
  ...
  function applyReferenceCard(card) {
    const presetUrl = presetUrlFor(card.referenceType);
    const inputUrls = card.referenceType === "multi-image-consistency-set" ? MULTI_IMAGE_REFERENCE_URLS : [presetUrl];
    setSelectedReferenceType(card.referenceType);
    setAcceptedReferences((current) => {
      const withoutSame = current.filter((item) => item.role !== card.referenceType);
      return [
        ...withoutSame,
        {
          type: "provider-preset",
          role: card.referenceType,
          url: presetUrl,
    ...
    setStatus(`Selected real provider reference: ${card.title}`);
    setSetupStage("reference-intake");
  }
  ...
  async function uploadReferenceFile(file, role = "uploaded-reference") {
    ...
    form.append("referenceType", role);
    ...
    setAcceptedReferences((current) => [...current.filter((item) => item.role !== role), next]);
  ...
  async function onReferenceFileChange(event) {
    ...
    if (!file) return;
    try {
      await uploadReferenceFile(file, selectedReferenceType || selectedVisualCard?.referenceTypes?.[0] || "uploaded-reference");
  ...
  async function startGuidedRecording() {
    try {
      ...
      const recorder = createGuidedRecorder(stream, async (blob) => {
        ...
        try {
          const data = await uploadGuidedCapture({ blob, projectId: fields.projectId || DEFAULT_PROJECT_ID, referenceType: recordingStep.referenceType });
          const next = {
            type: "video",
          ...
          setAcceptedReferences((current) => [...current.filter((item) => item.role !== next.role), next]);
          if (next.analysisId) setAnalysisId(next.analysisId);
    ...
    try {
      ...
      const response = await fetch("/api/admingeneration/routed-submit-v2", {
        ...
        body: JSON.stringify({
          ...
          inputImageUrls: anchor?.inputImageUrls || undefined,
          tailImageUrl: acceptedReferences.find((ref) => ref.role === "tail-end-frame")?.url || undefined,
          storyBible: setup.storyBible,
          metadata: {
            source: "admingeneration-real-reference-library",
            workflowMode,
            generationTypeId,
            visualCardId,
    ...
    <main className="opus-fit-shell">
      <div className="opus-board">
        ...
        <section className="opus-main compact-shell">
          ...
          {stage==="generate" ? <div className="movie-generate-state">
...
function VisualLibrary({ selectedId, onSelectReference, visualCards, selectedVisualId, onSelectVisual }) { const referenceCards=getReferenceSelectionCards(); return <div style={{position:"relative",width:"min(930px,99%)",margin:"0 auto",background:"#fff",color:"#111827",borderRadius:6,padding:8,boxShadow:"0 18px 50px rgba(0,0,0,.28)"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"4px 8px"}}><strong>Choose a strong reference image type</strong><small>Real provider images from stored library</small></div><div style={{display:"grid",gridTemplateColumns:"repeat(3,minmax(0,1fr))",gap:5}}>{referenceCards.map((card)=>{const url=presetUrlFor(card.referenceType);return <button key={card.id} onClick={()=>onSelectReference(card)} type="button" style={{border:selectedId===card.referenceType?"3px solid #7c3aed":"1px solid #d1d5db",background:"#fff",padding:0,cursor:"pointer",color:"#111827",textAlign:"center",minHeight:144,overflow:"hidden"}}><div style={{fontWeight:800,fontSize:12,padding:"4px 3px",whiteSpace:"nowrap"}}>{card.number}. {card.title}</div><div style={{height:92,backgroundImage:`url(${url})`,backgroundSize:"cover",backgroundPosition:"center",backgroundColor:"#e5e7eb"}} /><div style={{fontSize:10,padding:"4px 3px",lineHeight:1.15}}>{card.subtitle}</div></button>})}</div>{visualCards?.length?<div style={{marginTop:8,padding:8,borderTop:"1px solid #e5e7eb"}}><strong style={{fontSize:12}}>Visual direction presets</strong><div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:6}}>{visualCards.map((card)=><button key={card.id} onClick={()=>onSelectVisual(card)} type="button" style={{border:card.id===selectedVisualId?"2px solid #7c3aed":"1px solid #d1d5db",background:"#f8fafc",borderRadius:8,padding:"7px 9px",fontSize:11,color:"#111827"}}>{card.title}</button>)}</div></div>:null}</div>; }
function ReferenceIntake({ selectedReferenceType, selectedVisualCard, acceptedReferences, fileRef, recordingStep, setRecordingStep, isRecording, startGuidedRecording, stopGuidedRecording, previewVideoRef, continueToDirector }) { const required=selectedVisualCard?.referenceTypes||[]; return <div className="analysis-float-card" style={{position:"relative",width:"min(860px,96%)",margin:"0 auto"}}><strong>Guided Reference Intake</strong><p>Selected reference type: <b>{selectedReferenceType}</b></p><p>The stored provider image is already accepted. Upload or record a better user reference to replace it.</p><img src={presetUrlFor(selectedReferenceType)} alt={selectedReferenceType} style={{width:"min(460px,100%)",borderRadius:14,margin:"10px auto",display:"block"}}/><div className="prompt-chip-row" style={{justifyContent:"center",marginTop:12}}><button type="button" onClick={()=>fileRef.current?.click()}>Upload Replacement</button><button type="button" onClick={isRecording?stopGuidedRecording:startGuidedRecording}>{isRecording?"Stop Recording":"Record Guided Sample"}</button><button type="button" onClick={continueToDirector}>Continue to Final Setup</button></div><select value={recordingStep.id} onChange={(event)=>setRecordingStep(SELF_REFERENCE_CAPTURE_STEPS.find((step)=>step.id===event.target.value)||SELF_REFERENCE_CAPTURE_STEPS[0])} style={{marginTop:12}}><option value="face-close-up">Face close-up</option><option value="mid-shot-speaker">Mid-shot speaker</option><option value="gesture-sample">Gesture sample</option></select><p>{recordingStep.instruction}</p>{isRecording?<video ref={previewVideoRef} autoPlay muted playsInline style={{width:"min(360px,100%)",borderRadius:16}} />:null}<div style={{marginTop:10}}><strong>Visual preset also recommends:</strong><p>{required.length?required.join(", "):"No extra required references for this visual option."}</p><strong>Accepted:</strong><p>{acceptedReferences.length?acceptedReferences.map((ref)=>ref.role).join(", "):"None yet."}</p></div></div>; }
