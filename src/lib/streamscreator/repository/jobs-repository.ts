...
export async function createJob(input: CreateJobInput): Promise<RepositoryResult<Job>> {
  ...
  const { data, error } = await supabase.from(TABLE).insert({ project_id: input.project_id, type: input.type, status: input.status ?? 'queued', priority: input.priority ?? 0, payload: input.payload ?? null }).select().single<Job>();
...
export async function getJobById(jobId: string): Promise<RepositoryResult<Job>> {
  const supabase = createAdminClient();
  const { data, error } = await supabase.from(TABLE).select().eq('id', jobId).single<Job>();
...
export async function listJobsByProjectId(projectId: string): Promise<RepositoryResult<Job[]>> {
  const supabase = createAdminClient();
  const { data, error } = await supabase.from(TABLE).select().eq('project_id', projectId).order('created_at', { ascending: false }).returns<Job[]>();
