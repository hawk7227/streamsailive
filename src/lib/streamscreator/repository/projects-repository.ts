...
export async function createProject(input: CreateProjectInput): Promise<RepositoryResult<Project>> {
  ...
  const { data, error } = await supabase.from(TABLE).insert({ name: input.name, description: input.description ?? null, status: input.status ?? 'pending', metadata: input.metadata ?? null }).select().single<Project>();
...
export async function getProjectById(projectId: string): Promise<RepositoryResult<Project>> {
  const supabase = createAdminClient();
  const { data, error } = await supabase.from(TABLE).select().eq('id', projectId).single<Project>();
