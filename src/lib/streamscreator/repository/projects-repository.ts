...
export async function createProject(input: CreateProjectInput): Promise<RepositoryResult<Project>> {
  ...
  const { data, error } = await supabase.schema('streams').from('projects').insert({ name: input.name, description: input.description ?? null, status: input.status ?? 'pending', metadata: input.metadata ?? null }).select().single<Project>();
