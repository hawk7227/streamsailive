...
export async function createAsset(input: CreateAssetInput): Promise<RepositoryResult<Asset>> {
  ...
  const { data, error } = await supabase.schema('streams').from('assets').insert({ project_id: input.project_id, job_id: input.job_id ?? null, type: input.type, status: input.status ?? 'pending', filename: input.filename, mime_type: input.mime_type ?? null, size_bytes: input.size_bytes ?? null, storage_key: input.storage_key ?? null, public_url: input.public_url ?? null, metadata: input.metadata ?? null }).select().single<Asset>();
