...
// ─────────────────────────────────────────────────────────────────────────────
// All operations target streams.assets exclusively.
...
export async function createAsset(
  ...
  if (error !== null) {
    return repoErr(
      'ASSET_CREATE_FAILED',
      'Failed to create asset in streams.assets',
      error.message,
    );
  }
...
export async function listAssetsByProjectId(
  projectId: string,
): Promise<RepositoryResult<Asset[]>> {
  const supabase = createAdminClient();
  ...
  if (error !== null) {
    return repoErr(
      'ASSETS_LIST_FAILED',
      'Failed to list assets from streams.assets',
      error.message,
    );
  }
