...
export const STREAMS_TURN_CONTROLLER_VERSION = "streams-authoritative-turn-controller-v5";
...
export async function prepareAuthoritativeStreamsTurn(input: {
  scope: StreamsAIScope; sessionId: string; projectId?: string | null; userMessage: string; attachments?: unknown[]; turnId?: string | null; taskId?: string | null; requestedMode?: string | null; recentMessages?: StreamsContextPackage["recentMessages"]; attachmentText?: string; imageUrls?: string[]; selectedContext?: Record<string, unknown> | null; activeArtifact?: Record<string, unknown> | null; unresolvedTaskState?: Record<string, unknown> | null;
  ...
  return { controllerVersion: STREAMS_TURN_CONTROLLER_VERSION, parityProfileVersion: STREAMS_PARITY_PROFILE_VERSION, taskId, turnId, sessionId: input.sessionId, tenantId: input.scope.tenantId, userId: input.scope.userId, projectId, request: { text: input.userMessage, attachments, requestedMode: input.requestedMode || null }, intent, context, modelRoute, parityPlan, qualityPolicy: qualityPolicy(intent), state: "planning", createdAt: new Date().toISOString() };
}
...
function deterministicValidation(turn: StreamsAuthoritativeTurn, content: string) {
  if (!turn.qualityPolicy.requireDeterministicValidation) return { version: "not-required", accepted: true, critical: false, defects: [] } as StreamsDeterministicValidation;
  return validateDeterministicStreamsOutput({ instruction: turn.request.text, responseText: content, intent: turn.intent });
}

function judgmentForRepair(judgment: StreamsSemanticJudgment, deterministic: StreamsDeterministicValidation): StreamsSemanticJudgment {
  if (deterministic.accepted) return judgment;
  return {
    ...judgment,
    accepted: false,
    criticalDefect: true,
    repairable: true,
    defects: [...judgment.defects, ...deterministic.defects.map((defect) => ({ code: defect.code, severity: "critical" as const, message: defect.message, repairHint: defect.repairHint }))],
  };
}
...
export async function executeAuthoritativeStreamsTurn<TPersisted>(input: {
  ...
  while ((!judgment.accepted || !evidence.accepted || !deterministic.accepted) && repairAttempts < turn.qualityPolicy.maxRepairAttempts) {
    assertNotCancelled();
    if (!judgment.repairable && evidence.critical && deterministic.critical) break;
    repairAttempts += 1;
    emit("repairing", "Repairing response quality…");
    const repaired = await input.repairWithModel({ model: turn.modelRoute.repair.id, turn, candidate: selected, judgment: judgmentForRepair(judgment, deterministic), evidence, attempt: repairAttempts, signal: input.signal });
  ...
  return { turn, candidate: selected, judgment, evidence, deterministic, persisted, repairAttempts };
}
