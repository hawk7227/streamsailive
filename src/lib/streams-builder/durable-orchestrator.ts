...
export function createDurableRuntime(input: { projectId: string; workspaceId: string; jobId: string }, nowMs = Date.now()): DurableRuntimeRecord {
  const now = iso(nowMs);
  return {
    runtimeId: randomUUID(), projectId: input.projectId, workspaceId: input.workspaceId, jobId: input.jobId,
    status: "queued", stage: "plan", stageAttempt: 0, lease: null, checkpoints: [], commands: [], version: 1, createdAt: now, updatedAt: now,
  };
}
...
export async function claimRuntime(input: { store: DurableRuntimeStore; runtimeId: string; ownerId: string; ttlMs?: number; nowMs?: number; force?: boolean }): Promise<DurableRuntimeRecord> {
  ...
  const active = current.lease && Date.parse(current.lease.expiresAt) > nowMs;
  ...
  const next: DurableRuntimeRecord = {
    ...
    lease: { ownerId: input.ownerId, leaseId: randomUUID(), generation: (current.lease?.generation ?? 0) + 1, acquiredAt: iso(nowMs), heartbeatAt: iso(nowMs), expiresAt: iso(nowMs + Math.max(5_000, input.ttlMs ?? DEFAULT_TTL_MS)) },
...
export async function heartbeatRuntime(input: { store: DurableRuntimeStore; runtimeId: string; ownerId: string; ttlMs?: number; nowMs?: number }): Promise<DurableRuntimeRecord> {
  ...
  const next = { ...current, lease: { ...current.lease, generation: current.lease.generation + 1, heartbeatAt: iso(nowMs), expiresAt: iso(nowMs + Math.max(5_000, input.ttlMs ?? DEFAULT_TTL_MS)) }, version: current.version + 1, updatedAt: iso(nowMs) };
...
export async function appendRuntimeCommand(input: { store: DurableRuntimeStore; runtimeId: string; actorId: string; type: RuntimeCommand["type"]; payload?: Record<string, unknown>; nowMs?: number }): Promise<DurableRuntimeRecord> {
  const nowMs = input.nowMs ?? Date.now();
  const current = await input.store.get(input.runtimeId); if (!current) throw new Error("Runtime not found.");
  const command: RuntimeCommand = { id: randomUUID(), createdAt: iso(nowMs), actorId: input.actorId, type: input.type, payload: input.payload };
  const next = { ...current, commands: [...current.commands, command], version: current.version + 1, updatedAt: iso(nowMs) };
...
export function nextPendingCommand(record: DurableRuntimeRecord): RuntimeCommand | null { return record.commands.find((command) => !command.consumedAt) ?? null; }
...
export async function consumeRuntimeCommand(input: { store: DurableRuntimeStore; runtimeId: string; ownerId: string; commandId: string; nowMs?: number }): Promise<DurableRuntimeRecord> {
  const nowMs = input.nowMs ?? Date.now(); const current = await input.store.get(input.runtimeId);
  if (!current?.lease || current.lease.ownerId !== input.ownerId) throw new Error("Runtime command consumer does not own the lease.");
  const command = current.commands.find((item) => item.id === input.commandId); if (!command || command.consumedAt) throw new Error("Runtime command is missing or already consumed.");
  const commands = current.commands.map((item) => item.id === command.id ? { ...item, consumedAt: iso(nowMs) } : item);
  let status = current.status;
  if (command.type === "pause") status = "paused";
  if (command.type === "resume") status = "running";
  if (command.type === "cancel") status = "cancelled";
  const next = { ...current, commands, status, version: current.version + 1, updatedAt: iso(nowMs) };
...
export async function checkpointStage(input: { store: DurableRuntimeStore; runtimeId: string; ownerId: string; stage: RuntimeStage; inputDigest: string; output: Record<string, unknown>; evidenceUris?: string[]; nowMs?: number }): Promise<DurableRuntimeRecord> {
  ...
  const checkpoint: RuntimeCheckpoint = { id: randomUUID(), stage: input.stage, attempt: current.stage === input.stage ? current.stageAttempt + 1 : 1, createdAt: iso(nowMs), inputDigest: input.inputDigest, output: input.output, evidenceUris: input.evidenceUris ?? [] };
  const next = { ...current, stage: input.stage, stageAttempt: checkpoint.attempt, checkpoints: [...current.checkpoints, checkpoint], failure: undefined, version: current.version + 1, updatedAt: iso(nowMs) };
...
export async function failRuntime(input: { store: DurableRuntimeStore; runtimeId: string; ownerId: string; message: string; retryable: boolean; nowMs?: number }): Promise<DurableRuntimeRecord> {
  ...
  const next = { ...current, status: "failed" as const, failure: { stage: current.stage, message: input.message, retryable: input.retryable, occurredAt: iso(nowMs) }, lease: null, version: current.version + 1, updatedAt: iso(nowMs) };
...
export async function completeRuntime(input: { store: DurableRuntimeStore; runtimeId: string; ownerId: string; nowMs?: number }): Promise<DurableRuntimeRecord> {
  ...
  const next = { ...current, status: "completed" as const, stage: "complete" as const, lease: null, version: current.version + 1, updatedAt: iso(nowMs) };
