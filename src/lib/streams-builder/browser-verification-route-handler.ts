...
export async function handleBrowserVerificationPost(request: NextRequest) {
  try {
    ...
    const durableResult = {
      ...
      viewports: result.viewports.map((item) => ({
        ...item,
        screenshot: item.screenshot ? { ...item.screenshot, dataUrl: undefined } : undefined,
      })),
      evidenceAssets,
      checkpointId: body.checkpointId || null,
      previewId: body.previewId || null,
    ...
    await jobs.update(scope, jobId, {
      ...
      metadata: {
        browserVerification: durableResult,
