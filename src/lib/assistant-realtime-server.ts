...
import { AssistantWebSocketAdapter, type RealtimeSocket } from "./assistant-websocket-adapter";
...
export type AssistantRealtimeServerOptions = {
  path?: string;
  logger?: Logger;
  getInitialContext?: (request: IncomingMessage) => Promise<Record<string, unknown>> | Record<string, unknown>;
  authorize?: (request: IncomingMessage) => Promise<{ ok: true; sessionId?: string } | { ok: false; code?: number; reason: string }> | { ok: true; sessionId?: string } | { ok: false; code?: number; reason: string };
};
...
class NodeWebSocketAdapter implements RealtimeSocket {
  ...
  addEventListener(
    ...
    listener: (event: { data?: unknown; code?: number; reason?: string }) => void | Promise<void>,
  ): void {
    if (type === "message") {
      this.socket.on("message", async (data: RawData) => {
        await listener({ data: normalizeRawData(data) });
      });
      return;
    }
    ...
    if (type === "close") {
      this.socket.on("close", async (code, reason) => {
        await listener({ code, reason: reason.toString("utf8") });
      });
      return;
    }

    this.socket.on("error", async (error) => {
      await listener({ data: error });
    });
  }
}
