...
export default function GlobalNavigationRail() {
  ...
  function activate(item: NavItem) {
    ...
    if (item.action === "Home") { openChat(); return; }
    if (item.action === "Projects") { window.dispatchEvent(new CustomEvent("streams-ai:open-project-creation")); return; }
    if (item.action === "Files" || item.action === "Assets") { setTrayTab("Assets"); return; }
    if (item.action === "Tasks") { setTrayTab("Tasks"); return; }
    if (item.action === "History") { setTrayTab("Versions"); return; }
    ...
    <nav className="globalNavigationRail" aria-label="StreamsAI global navigation">
      ...
      <div className="railItems">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <button key={item.action} type="button" className={active ? "railItem active" : "railItem"} onClick={() => activate(item)} aria-current={active ? "page" : undefined} aria-label={item.label} title={item.label}>
      ...
      <style jsx>{`
        ...
        .railBrand,.railNew,.railItem{border:0;cursor:pointer;color:#cbd5e1;background:transparent}.railBrand{width:40px;height:40px;justify-self:center;border:1px solid rgba(148,163,184,.28);border-radius:12px;background:#0f1a2d;font-size:15px;font-weight:900}.railNew{width:40px;height:40px;justify-self:center;border-radius:12px;background:#172554;color:#dbeafe;font-size:21px;font-weight:800}
        .railItems{min-height:0;overflow-y:auto;overflow-x:hidden;display:grid;align-content:start;gap:4px;padding-bottom:10px;scrollbar-width:none}.railItems::-webkit-scrollbar{display:none}.railItem{min-height:48px;border-radius:12px;display:grid;place-items:center;align-content:center;gap:2px;padding:5px 2px}.railItem span{font-size:15px;font-weight:900;line-height:1}.railItem small{font-size:8px;line-height:1.1;white-space:nowrap}.railItem:hover{background:#101832;color:#fff}.railItem.active{background:#172554;color:#bfdbfe;box-shadow:inset 0 0 0 1px rgba(96,165,250,.18)}
