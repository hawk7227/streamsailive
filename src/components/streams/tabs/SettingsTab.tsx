    ...
    <div style={{ height: "100%", overflowY: "auto" }}>
      <div style={{ maxWidth: 700, margin: "0 auto", padding: "24px 20px", display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 11, color: C.t3 }}>API keys and model defaults. Provider names shown here only — everywhere else uses Streams brand names.</div>
        </div>
        ...
        <div style={{ background: C.bg2, border: `1px solid ${C.bdr}`, borderRadius: R.r3, overflow: "hidden" }}>
          <div style={{ padding: "12px 18px", borderBottom: `1px solid ${C.bdr}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ fontSize: 12, fontWeight: 500, color: C.t1 }}>API keys</div>
            <span style={{ fontSize: 9, color: C.t4, padding: "2px 8px", borderRadius: R.pill, background: C.surf, border: `1px solid ${C.bdr}` }}>encrypted at rest</span>
          </div>
          {keys.map((key: ApiKey, i: number) => (
            <div key={key.provider} style={{ padding: "14px 18px", borderBottom: i < keys.length - 1 ? `1px solid ${C.bdr}` : "none" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                ...
                <div>
                  <span style={{ fontSize: 12, color: C.t1, fontWeight: 500 }}>{key.label}</span>
                  <span style={{ fontSize: 10, color: C.t4, marginLeft: 8 }}>{key.provider}</span>
                </div>
                <span style={{ fontSize: 10, color: statusColor(key.status), fontWeight: 500 }}>
                  {statusLabel(key.status)}
                </span>
              </div>
              <div style={{ fontSize: 10, color: C.t4, marginBottom: 8, lineHeight: 1.5 }}>{key.hint}</div>
              <div style={{ display: "flex", gap: 8 }}>
                <div style={{ flex: 1, position: "relative" }}>
                  <input
                    ...
                    style={{ width: "100%", background: C.bg3, border: `1px solid ${C.bdr}`, borderRadius: R.r1, padding: "8px 36px 8px 10px", color: C.t1, fontSize: 12, fontFamily: "inherit", outline: "none" }}
                  />
                  <button onClick={() => setShowKeys((s: boolean[]) => s.map((x: boolean, idx: number) => idx === i ? !x : x))} style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", color: C.t4, cursor: "pointer", fontSize: 12 }}>
                    {showKeys[i] ? "◉" : "○"}
                  </button>
                </div>
                <button onClick={() => testKey(i)} disabled={key.status === "testing"} style={{ padding: "8px 14px", borderRadius: R.r1, background: C.surf, border: `1px solid ${C.bdr}`, color: C.t2, fontSize: 11, fontFamily: "inherit", cursor: "pointer", flexShrink: 0, transition: `all ${DUR.fast} ${EASE}` }}>
                  Test
                </button>
              </div>
            </div>
        ...
        <div style={{ background: C.bg2, border: `1px solid ${C.bdr}`, borderRadius: R.r3, overflow: "hidden" }}>
          <div style={{ padding: "12px 18px", borderBottom: `1px solid ${C.bdr}` }}>
            <div style={{ fontSize: 12, fontWeight: 500, color: C.t1 }}>Default models</div>
            <div style={{ fontSize: 10, color: C.t4, marginTop: 2 }}>Used when no model is explicitly selected in the Generate tab.</div>
          </div>
          {models.map((m: typeof models[0], i: number) => (
            <div key={m.mode} style={{ padding: "12px 18px", borderBottom: i < models.length - 1 ? `1px solid ${C.bdr}` : "none", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <span style={{ fontSize: 12, color: C.t1 }}>{m.mode}</span>
              <select
                value={m.current}
                ...
                style={{ background: C.bg3, border: `1px solid ${C.bdr}`, borderRadius: R.r1, padding: "6px 10px", color: C.t1, fontSize: 11, fontFamily: "inherit", outline: "none" }}
              >
        ...
        <div style={{ background: C.bg2, border: `1px solid ${C.bdr}`, borderRadius: R.r3, padding: "14px 18px" }}>
          <div style={{ fontSize: 12, fontWeight: 500, color: C.t1, marginBottom: 4 }}>Quality preset</div>
          <div style={{ fontSize: 10, color: C.t4, marginBottom: 12 }}>Maps to model tier + parameters across all generation modes.</div>
          <div style={{ display: "flex", gap: 8 }}>
            {(["fast","standard","pro"] as const).map(q => (
              <button key={q} onClick={() => setQuality(q)} style={{
                ...
                fontSize: 12, fontFamily: "inherit", cursor: "pointer", fontWeight: quality === q ? 500 : 400,
        ...
        <div style={{ background: C.bg2, border: `1px solid ${C.bdr}`, borderRadius: R.r3, overflow: "hidden" }}>
          <div style={{ padding: "12px 18px", borderBottom: `1px solid ${C.bdr}` }}>
            <div style={{ fontSize: 12, fontWeight: 500, color: C.t1 }}>Cost guardrails</div>
            <div style={{ fontSize: 10, color: C.t4, marginTop: 2 }}>System refuses to submit if threshold exceeded.</div>
          </div>
          {[
            ...
            <div key={row.label} style={{ padding: "12px 18px", borderBottom: `1px solid ${C.bdr}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <input
                  ...
                  style={{ width: 80, background: C.bg3, border: `1px solid ${C.bdr}`, borderRadius: R.r1, padding: "6px 10px", color: C.t1, fontSize: 12, fontFamily: "inherit", outline: "none", textAlign: "right" }}
                />
              </div>
            </div>
          ))}
          <div style={{ padding: "12px 18px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
              <div style={{ fontSize: 12, color: C.t1 }}>"Made with Streams" watermark</div>
              <div style={{ fontSize: 10, color: C.t4, marginTop: 2 }}>Shown on shared links. Disable for brand accounts.</div>
            </div>
            <button onClick={() => setWatermark(!watermark)} style={{
              ...
              fontSize: 11, fontFamily: "inherit", cursor: "pointer", transition: `all ${DUR.fast} ${EASE}`,
            }}>
              {watermark ? "On" : "Off"}
            </button>
          </div>
