...
type ActionStatus = "idle" | "loading" | "success" | "error" | "blocked";
...
export default function VideoEditorTab({ analysisId, genLogId, videoUrl }: { analysisId?: string | null; genLogId?: string | null; videoUrl?: string | null; }) {
  ...
  const [timeline, setTimeline] = useState<Array<{ id: string; label: string; state: string }>>([]);
  ...
  const actionInv = useMemo(() => ACTIONS, []);
  ...
  async function runAction(id: string) {
    ...
    try {
      ...
      const res = await fetch(action.route, { method: action.method ?? "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
      const data = (await res.json()) as Record<string, unknown>;
      if (!res.ok) throw new Error(String(data.error ?? `${action.label} failed`));

      setResult(data);
      setStatus((s) => ({ ...s, [id]: "success" }));
      setTimeline((t) => [{ id: crypto.randomUUID(), label: action.label, state: "success" }, ...t].slice(0, 12));
    } catch (e) {
      const msg = e instanceof Error ? e.message : "Unknown error";
      setError(msg);
      setStatus((s) => ({ ...s, [id]: "error" }));
      setTimeline((t) => [{ id: crypto.randomUUID(), label: action.label, state: `error: ${msg}` }, ...t].slice(0, 12));
    }
  }

  return <div data-testid="streams-editor" style={{ display: "grid", gridTemplateRows: "56px minmax(0,1fr) 220px", height: "100%", gap: 8 }}>
    <div style={{ border: `1px solid ${C.bdr}`, borderRadius: R.r2, padding: "8px 12px", display: "flex", justifyContent: "space-between" }}>
      ...
      <div style={{ fontSize: 12, color: C.t3 }}>analysis:{analysisId ?? "not analyzed"} · generation:{genLogId ?? "missing"}</div>
    </div>
    <div style={{ display: "grid", gridTemplateColumns: "minmax(240px,300px) minmax(0,1fr) minmax(280px,360px)", gap: 8, minHeight: 0 }}>
      <section data-testid="editor-left" style={{ border: `1px solid ${C.bdr}`, borderRadius: R.r2, padding: 8, overflow: "auto" }}>
      ...
      <section data-testid="editor-right" style={{ border: `1px solid ${C.bdr}`, borderRadius: R.r2, padding: 8, overflow: "auto" }}>
        <h4>Actions</h4>
        {actionInv.map((a) => <div key={a.id} style={{ border: `1px solid ${C.bdr}`, borderRadius: R.r1, padding: 8, marginBottom: 8 }}>
          ...
          {a.available ? <button onClick={() => { setActiveAction(a.id); void runAction(a.id); }} style={{ marginTop: 8 }}>Run</button> : <div style={{ fontSize: 12, color: C.t3, marginTop: 8 }}>{a.blockedReason}</div>}
    ...
    <section data-testid="editor-timeline" style={{ border: `1px solid ${C.bdr}`, borderRadius: R.r2, padding: 8, overflow: "auto" }}>
      ...
      {timeline.length === 0 ? <div style={{ fontSize: 12, color: C.t3 }}>No jobs yet.</div> : timeline.map((x) => <div key={x.id} style={{ fontSize: 12 }}>{x.label}: {x.state}</div>)}
    </section>
  </div>;
}
