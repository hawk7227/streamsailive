...
import {
  buildStoryDirectorPlan,
  type StoryDirectorInput,
  type StoryDirectorPlan,
} from "@/lib/pipeline-test/storyDirector";
...
export default function StoryDirectorPanel({
  ...
  function setField<K extends keyof StoryDirectorInput>(
    key: K,
    value: StoryDirectorInput[K]
  ) {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "grid", gap: 10 }}>
        ...
        />
        <input
          value={form.visualStrategy}
          onChange={(e) => setField("visualStrategy", e.target.value)}
        ...
        <div style={{ marginTop: 12 }}>
          ...
          <ul>
            {plan.scenes.map((scene) => (
              <li key={scene.id}>
                <strong>{scene.title}:</strong> {scene.beat}
              </li>
            ))}
          </ul>
        </div>
