...
export const JsonViewer = ({ data, nodeLabel }: JsonViewerProps) => {
  ...
  const renderJson = (obj: unknown, path: string = ""): React.ReactNode => {
    if (typeof obj !== 'object' || obj === null) {
      const isCopied = copiedPath === path;
      const strVal = String(obj);
      const isImage = typeof obj === 'string' && (
        obj.startsWith('data:image/') || /^https?:\/\/.*\.(jpeg|jpg|gif|png|webp)$/i.test(obj)
      );
      if (isImage) {
        return (
          <div key={path} className="group relative">
            <div className="flex items-start gap-2 hover:bg-white/5 p-1 rounded cursor-pointer transition-colors"
              onClick={(e) => handleCopy(path, e)}>
              <span className="text-gray-500 shrink-0">{path.split('.').pop()}:</span>
              <span className="text-emerald-400 break-all line-clamp-1">{strVal}</span>
            </div>
            <div className="mt-1 ml-4 mb-2">
              <img src={obj as string} alt="Preview" className="max-w-full h-auto max-h-32 rounded border border-white/10 object-contain bg-[#050505]" />
            </div>
          </div>
        );
      }
      return (
        ...
        >
          <span className="text-gray-500 shrink-0">{path.split('.').pop()}:</span>
          <span className="text-emerald-400 break-all">{strVal}</span>
