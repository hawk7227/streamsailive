...
export default function Hero() {
  return (
    <section id="product" className="relative isolate overflow-hidden px-5 pb-14 pt-28 sm:px-8 sm:pb-20 sm:pt-32 lg:px-10 lg:pb-24 lg:pt-36">
      ...
      <div className="mx-auto grid w-full max-w-7xl items-center gap-12 lg:grid-cols-[minmax(0,1.05fr)_minmax(380px,.95fr)] lg:gap-8">
        <div className="max-w-3xl">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row">
            ...
            <Link href="/streams-ai" className="inline-flex min-h-12 items-center justify-center rounded-xl border border-white/15 px-6 py-3 text-sm font-semibold text-white transition-colors hover:bg-white/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50">See How It Works <span className="ml-3" aria-hidden="true">▶</span></Link>
          </div>
        </div>
      ...
      <div className="mx-auto mt-12 grid w-full max-w-7xl grid-cols-1 gap-8 border-t border-white/8 pt-9 sm:grid-cols-3 lg:mt-8">
        {pillars.map(({ title, description, icon: Icon, accent, glow }) => (
          <div key={title} className="flex items-start gap-4">
            ...
            <div><h2 className="text-base font-semibold text-white">{title}</h2><p className="mt-1.5 max-w-xs text-sm leading-6 text-slate-500">{description}</p></div>
          </div>
        ))}
      </div>
    </section>
