...
type Accent = "blue" | "purple" | "pink" | "rose" | "cyan" | "green" | "orange";
...
type Capability = {
  ...
  fields: string[];
  images: string[];
  testimonials: { quote: string; result: string; built: string }[];
};

const accentStyles: Record<Accent, { border: string; glow: string; badge: string; button: string; chip: string }> = {
  blue: {
...
const capabilities: Capability[] = [
  {
    ...
    description: "Takes out the guesswork when you want to start, grow, or generate multiple income streams.",
    price: "$29/mo",
    ...
    resultLabels: ["Business Plan", "Website", "Branding", "Growth", "Ad Plan"],
  ...
  },
  {
    ...
    description: "Turn prompts into brand images, product visuals, ads, thumbnails, and website graphics.",
    price: "$19/mo",
    plan: "Visual Creator",
    cta: "Create Image",
    ...
    resultLabels: ["Product", "Ad Creative", "Brand", "Thumbnail", "Web Graphic"],
  ...
  },
  {
    ...
    resultLabels: ["Motion", "Promo", "Selfie", "Social", "Effects"],
  ...
  },
  {
    ...
    resultLabels: ["Ad Video", "Story", "Promo", "Explainer", "Shorts"],
  ...
  },
  {
    ...
    resultLabels: ["Dance", "Motivate", "FaceTime", "Funny", "Background", "Cinematic"],
  ...
  },
  {
    ...
    resultLabels: ["Plan", "Generate", "Edit", "Stitch", "Export"],
...
function CapabilityCard({ capability }: { capability: Capability }) {
  ...
  const previous = () => {
    setActiveIndex((current) => (current === 0 ? capability.testimonials.length - 1 : current - 1));
  };
  ...
  return (
    <article className="min-w-0">
      <div className="mb-3 grid min-h-[112px] grid-cols-[30px_1fr_30px] items-center gap-2">
        <button
          type="button"
          onClick={previous}
          className="grid h-7 w-7 place-items-center rounded-full border border-white/15 bg-white/[0.06] text-white transition hover:bg-white/10"
          aria-label={`Previous ${capability.title} testimonial`}
        >
          <ArrowLeft className="h-3.5 w-3.5" />
        </button>
        <div className="min-w-0">
          <p className="break-words text-base font-black leading-tight tracking-[-0.03em] text-white">“{activeStory.quote}”</p>
          <p className="mt-1.5 break-words text-[11px] font-semibold leading-4 text-slate-300">{activeStory.result}</p>
          <p className="mt-1 break-words text-[11px] font-bold leading-4 text-cyan-200">{activeStory.built}</p>
        </div>
        <button
          type="button"
          onClick={next}
      ...
      <div className={`relative h-full min-h-[520px] overflow-hidden rounded-[24px] border ${accent.border} bg-[#080c18] shadow-2xl ${accent.glow}`}>
        <div className="absolute left-1/2 top-2.5 z-10 h-4 w-20 -translate-x-1/2 rounded-full bg-black/80" />
        <div className="relative z-0 flex h-full min-h-[520px] flex-col p-3 pt-8">
          <div className="mb-2.5 flex items-start gap-2.5">
            <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br ${accent.badge} text-lg font-black text-white shadow-lg`}>
              {capability.number}
            </div>
            <div className="min-w-0 flex-1">
              <div className="mb-0.5 flex items-center justify-between gap-2">
                <h3 className="truncate text-base font-black leading-tight text-white">{capability.title}</h3>
                <Icon className="h-4 w-4 shrink-0 text-white/70" />
              </div>
              <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-white/45">{capability.eyebrow}</p>
            </div>
          </div>

          <p className="mb-2 line-clamp-2 text-[11px] leading-4 text-slate-300">{capability.description}</p>

          <div className="mb-2 flex items-center justify-end text-right">
            <div>
              <div className="text-sm font-black text-white">{capability.price}</div>
              <div className="text-[9px] font-bold uppercase tracking-[0.12em] text-slate-400">{capability.plan}</div>
            </div>
          </div>

          {!workspaceOpen ? (
            <div className="mb-2.5 flex-1 rounded-xl border border-white/10 bg-slate-950/70 p-2">
              <div className="mb-2 flex items-center justify-between px-1">
                <span className="text-[8px] font-black uppercase tracking-[0.16em] text-cyan-200">Before → results</span>
                <span className="text-[8px] font-bold uppercase tracking-[0.12em] text-slate-500">Default</span>
              </div>
              <div className="grid h-[250px] grid-cols-[1.08fr_1.55fr] gap-2">
                <div className={`overflow-hidden rounded-xl border ${accent.border} bg-slate-950 shadow-sm`}>
                  <img src={previewImage} alt="" className="h-[206px] w-full object-cover brightness-[.84] contrast-125 saturate-150" />
                  <div className={`border-t px-2 py-2 text-center text-[9px] font-black uppercase tracking-[0.08em] ${accent.chip}`}>
                    {capability.sourceLabel}
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-1.5">
                  {capability.resultLabels.slice(0, 6).map((label, index) => (
                    <div key={label} className={`overflow-hidden rounded-lg border ${accent.border} bg-slate-950/80 shadow-sm`}>
                      <img
                        src={capability.images[(index + 1) % capability.images.length]}
                        alt=""
                        className="h-[58px] w-full object-cover brightness-[.84] contrast-125 saturate-150"
                      />
                      <div className={`min-h-[28px] px-1.5 py-1 text-center text-[8px] font-black leading-[1.05] ${accent.chip}`}>
                        {label}
                      </div>
                    </div>
                  ))}
            ...
            <div className="mb-2.5 flex-1 rounded-xl border border-white/10 bg-slate-950/70 p-2.5">
              <div className="mb-2 flex items-center justify-between">
                <div className="text-[11px] font-black uppercase tracking-[0.12em] text-white">Workspace Inputs</div>
                <button type="button" onClick={() => setWorkspaceOpen(false)} className="text-[10px] font-bold uppercase tracking-[0.12em] text-slate-400 hover:text-white">
                  Back
                </button>
              </div>
              <div className="grid gap-1.5">
                {capability.fields.slice(0, 5).map((field, index) => (
                  <label key={field}>
                    <span className="mb-1 block text-[8px] font-black uppercase tracking-[0.14em] text-slate-500">{field}</span>
                    {index === 0 ? (
                      <textarea className="h-12 w-full resize-none rounded-lg border border-white/10 bg-white/[0.04] p-2 text-[11px] leading-4 text-white outline-none placeholder:text-slate-500" placeholder={`Enter ${field.toLowerCase()}...`} />
                    ) : (
                      <div className="flex h-8 items-center justify-between rounded-lg border border-white/10 bg-white/[0.04] px-2.5 text-[11px] font-semibold text-slate-300">
              ...
              <div className="mt-2 rounded-lg border border-white/10 bg-white/[0.04] p-2 text-[10px] leading-4 text-slate-300">
                Preview mode only. Final generation unlocks after account setup.
              </div>
            </div>
          )}
          ...
          <div className="mt-auto grid grid-cols-2 gap-2">
            <button
              ...
              className={`rounded-xl bg-gradient-to-r ${accent.button} px-3 py-2.5 text-xs font-black text-white shadow-lg transition active:scale-[.98]`}
            >
              {workspaceOpen ? "Show Results" : capability.cta}
            </button>
            <Link href={capability.href} className="flex items-center justify-center rounded-xl border border-white/15 bg-white/[0.06] px-3 py-2.5 text-xs font-black text-white transition hover:bg-white/[0.1]">
...
export default function Features() {
  return (
    <section className="relative overflow-visible bg-[#050711] py-16 sm:py-20" id="features">
      ...
      <div className="relative mx-auto w-full max-w-[1800px] px-4 sm:px-6">
        <div className="mb-10 text-center">
          <div className="mx-auto mb-4 inline-flex items-center gap-2 rounded-full border border-cyan-300/20 bg-cyan-300/10 px-4 py-2 text-xs font-black uppercase tracking-[0.18em] text-cyan-200">
        ...
        <div className="grid w-full min-w-0 gap-x-5 gap-y-10 md:grid-cols-2 xl:grid-cols-4">
