...
function allMatches(value: string, query: string): MatchRange[] { const needle = query.trim().toLowerCase(); if (!needle) return []; const haystack = value.toLowerCase(); const found: MatchRange[] = []; let index = 0; while (index <= haystack.length) { const next = haystack.indexOf(needle, index); if (next < 0) break; const pos = positionFromOffset(value, next); found.push({ start: next, end: next + needle.length, line: pos.line, column: pos.column }); index = next + Math.max(needle.length, 1); } return found; }
...
export default function RuntimeCodeEditor({ value, filePath, sha, onChange, onSelectionChange, highlightRange }: RuntimeCodeEditorProps) {
  ...
  function showCopyNotice(message: string) { setCopyNotice(message); if (copyTimerRef.current) window.clearTimeout(copyTimerRef.current); copyTimerRef.current = window.setTimeout(() => setCopyNotice(""), 2200); }
  function syncScroll() { if (textareaRef.current) setEditorScrollTop(textareaRef.current.scrollTop); if (gutterRef.current && textareaRef.current) gutterRef.current.scrollTop = textareaRef.current.scrollTop; }
  ...
  async function copyText(text: string, label: string) { if (!text) { report("Nothing to copy"); showCopyNotice("Nothing to copy"); return; } try { await navigator.clipboard.writeText(text); } catch { const textarea = textareaRef.current; textarea?.focus(); document.execCommand("copy"); } const message = `${label} copied`; report(message); showCopyNotice(`✓ ${message}`); }
  ...
  return (
    <section className="runtimeCodeEditor" aria-label={`Runtime code editor for ${filePath}`}>
      ...
      <div className="selectionBar"><span>{activeMatch ? `Search highlight ${findIndex + 1} of ${matches.length} · line ${activeMatch.line}, col ${activeMatch.column}` : selection?.text ? `Selected lines ${selection.startLine}-${selection.endLine}, columns ${selection.startColumn}-${selection.endColumn}` : highlightRange ? `Highlighted lines ${highlightRange.startLine}-${highlightRange.endLine}` : "Click in code to select a range"}</span><span>{status}</span></div>
      <div className="editorFrame"><pre ref={gutterRef} className="lineGutter" aria-hidden="true">{numbers}</pre><div className="codeLayer"><div className="searchLineLayer" style={{ transform: `translateY(${-editorScrollTop}px)` }}>{Array.from({ length: totalLines }, (_, index) => { const line = index + 1; const isSearch = matchedLines.has(line); const isActive = activeMatch?.line === line; return <div key={line} className={isActive ? "searchLine active" : isSearch ? "searchLine match" : "searchLine"} />; })}</div><textarea ref={textareaRef} value={value} onChange={handleChange} onSelect={updateSelection} onKeyUp={updateSelection} onClick={updateSelection} onScroll={syncScroll} onKeyDown={handleKeyDown} spellCheck={false} /></div></div>
      <style jsx>{`.runtimeCodeEditor{position:relative;height:100%;min-height:0;display:grid;grid-template-rows:auto auto auto auto auto auto minmax(0,1fr);background:#0d1117;color:#e6edf3;overflow:hidden;border:1px solid #30363d;border-radius:12px}.codeToolbar{min-width:0;display:flex;align-items:center;justify-content:space-between;gap:12px;padding:8px 10px;background:#f6f8fa;border-bottom:1px solid #d0d7de;color:#24292f}.fileMeta{min-width:0;display:grid;gap:2px}.fileMeta b{min-width:0;color:#24292f;font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.fileMeta span{color:#57606a;font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.iconTools{display:flex;align-items:center;gap:8px}.iconTools button{height:32px;min-width:34px;border:1px solid #d0d7de;border-radius:10px;background:#f6f8fa;color:#57606a;font-size:18px;font-weight:800;cursor:pointer}.utilityBar{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:7px 12px;background:#fff;border-bottom:1px solid #d8dee4;color:#57606a;font-size:12px}.cursorMeta,.historyMeta{display:flex;align-items:center;gap:14px;min-width:0}.historyMeta button{border:0;background:transparent;color:#24292f;font-weight:900;cursor:pointer;font-size:13px}.editMenu{display:flex;gap:6px;flex-wrap:wrap;padding:8px 10px;background:#f6f8fa;border-bottom:1px solid #d0d7de}.editMenu button,.findBar button,.markBar button,.searchHitBar button{height:30px;border:1px solid #d0d7de;border-radius:7px;background:#fff;color:#24292f;font-size:11px;font-weight:800;padding:0 9px;cursor:pointer}.findBar{display:grid;grid-template-columns:minmax(160px,1fr) auto auto auto auto;align-items:center;gap:6px;padding:8px 10px;background:#fff8c5;border-bottom:1px solid #d4a72c;color:#24292f;font-size:11px}.findBar input{height:30px;border:1px solid #d0d7de;border-radius:8px;background:#fff;color:#24292f;padding:0 10px;outline:none}.searchHitBar{display:flex;align-items:center;gap:6px;flex-wrap:wrap;padding:6px 10px;background:#3b2f04;border-bottom:1px solid #d4a72c;color:#fde68a;font-size:11px}.searchHitBar b{color:#fff}.searchHitBar button{height:24px;background:#fff8c5;border-color:#facc15}.searchHitBar button.active{background:#facc15;color:#111827;box-shadow:0 0 0 2px rgba(250,204,21,.35)}.copyToast{position:absolute;right:14px;top:116px;z-index:30;border:1px solid rgba(34,197,94,.45);border-radius:999px;background:#052e1a;color:#bbf7d0;padding:8px 12px;font-size:12px;font-weight:900;box-shadow:0 12px 30px rgba(0,0,0,.35)}.markBar{display:flex;gap:6px;flex-wrap:wrap;padding:6px 10px;background:#101826;border-bottom:1px solid #30363d}.markBar button.highlight{background:#fff8c5}.markBar button.circle{border-radius:999px}.markBar button.underline{text-decoration:underline}.selectionBar{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:6px 12px;background:#0d1117;border-bottom:1px solid #30363d;color:#8b949e;font-size:11px}.selectionBar span{min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.editorFrame{min-width:0;min-height:0;display:grid;grid-template-columns:54px minmax(0,1fr);overflow:hidden;background:#0d1117}.lineGutter{margin:0;padding:10px 8px 40px 0;background:#0d1117;border-right:1px solid #30363d;color:#6e7681;text-align:right;font:12px/19px ui-monospace,SFMono-Regular,Consolas,Liberation Mono,Menlo,monospace;overflow:hidden;user-select:none}.codeLayer{position:relative;min-width:0;min-height:0;overflow:hidden;background:#0d1117}.searchLineLayer{position:absolute;left:0;right:0;top:0;z-index:1;pointer-events:none;padding-top:10px}.searchLine{height:19px;border-left:3px solid transparent;box-sizing:border-box}.searchLine.match{background:rgba(250,204,21,.16);border-left-color:rgba(250,204,21,.8);box-shadow:inset 0 0 0 1px rgba(250,204,21,.08)}.searchLine.active{background:rgba(250,204,21,.36);border-left-color:#f97316;box-shadow:inset 0 0 0 1px rgba(250,204,21,.35),0 0 16px rgba(250,204,21,.18)}.codeLayer textarea{position:relative;z-index:2;width:100%;height:100%;min-width:0;min-height:0;resize:none;border:0;outline:0;background:transparent;color:#e6edf3;padding:10px 12px 40px;font:12px/19px ui-monospace,SFMono-Regular,Consolas,Liberation Mono,Menlo,monospace;white-space:pre;overflow:auto;tab-size:2;box-sizing:border-box}.codeLayer textarea::selection{background:rgba(250,204,21,.95);color:#111827}`}</style>
    </section>
  );
}
