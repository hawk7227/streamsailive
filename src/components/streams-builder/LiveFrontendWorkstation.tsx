...
function normalizeRoute(value: string) { const route = String(value || "/").trim(); return route.startsWith("/") ? route : `/${route}`; }
function liveUrlFor(repo: string, route: string) { if (route.startsWith("/streams-builder/preview/")) return route; const app = (repo || "").split("/").pop() || ""; return app ? `https://${app}.vercel.app${route}` : route; }
function label(tab: Tab) { return tab === "frontend" ? "Frontend UI" : tab === "code" ? "Code Editor" : tab === "diff" ? "Diff" : tab === "logs" ? "Logs" : tab === "media" ? "Media" : "DevTools"; }
function levelFor(value: string): ProofLine["level"] { const text = value.toLowerCase(); if (/failed|blocked|error|unauthorized/.test(text)) return "error"; if (/approval|awaiting|unproven|skipped/.test(text)) return "warning"; if (/passed|completed|verified|ready|queued|mounted|pulled|applied/.test(text)) return "success"; return "info"; }
function numberValue(...values: unknown[]) { for (const value of values) { const parsed = Number(value); if (Number.isFinite(parsed) && parsed > 0) return Math.floor(parsed); } return 1; }
function stringValue(...values: unknown[]) { for (const value of values) if (typeof value === "string" && value.length) return value; return ""; }
...
export default function LiveFrontendWorkstation({ activeFile, toolbar, onContentChange }: Props) {
  ...
  const ready = Boolean(activeFile.path || route.startsWith("/streams-builder/preview/"));
  ...
  function addProof(message: string) { setProof((items) => [...items.slice(-79), { id: `${Date.now()}-${Math.random().toString(36).slice(2)}`, message, at: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit" }), level: levelFor(message) }]); }
  function setDraft(next: string) { draftRef.current = next; setCodeDraft(next); onContentChange?.(next); }

  async function animatePatch(frame: PatchFrame) {
    if (frame.path && activeFile.path && frame.path !== activeFile.path) { addProof(`Patch blocked: worker targeted ${frame.path}, visible lock is ${activeFile.path}.`); return; }
    ...
    for (const line of changed) {
      ...
      const index = line - 1;
      if (index >= afterLines.length) working.splice(index, 1); else if (index >= working.length) working.push(afterLines[index]); else working[index] = afterLines[index];
  ...
  useEffect(() => { animationToken.current += 1; setCodeDraft(activeFile.content || ""); draftRef.current = activeFile.content || ""; setBaseline(activeFile.content || ""); setHighlightRange(null); if (ready && activeFile.path) addProof(`Source mounted: ${activeFile.repo}@${activeFile.branch}:${activeFile.path}`); }, [activeFile.repo, activeFile.branch, activeFile.path, activeFile.sha, activeFile.content, ready]);

  useEffect(() => {
    function onSummary(event: Event) { const detail = (event as CustomEvent<{ message?: string }>).detail; if (detail?.message) addProof(detail.message); }
    function onExternalPatch(event: Event) { const detail = (event as CustomEvent<RuntimeEvent>).detail || {}; const frame = eventFrame(detail, activeFile.path); if (frame) void animatePatch(frame); }
    function onRuntimeJob(event: Event) {
      const detail = (event as CustomEvent<{ jobId?: string }>).detail; if (!detail?.jobId) return;
      ...
      async function poll() {
        ...
        } catch (error) { if (!stopped) addProof(error instanceof Error ? error.message : "Runtime proof unavailable"); }
      }
      void poll();
    ...
    return () => { window.removeEventListener("streams-builder-summary-event", onSummary); window.removeEventListener("streams-builder:runtime-job", onRuntimeJob); window.removeEventListener("streams-builder:worker-patch", onExternalPatch); };
  }, [activeFile.path]);

  const diff = useMemo(() => {
    const before = baseline.split("\n"); const after = codeDraft.split("\n"); const max = Math.max(before.length, after.length); const rows: string[] = [];
    for (let index = 0; index < max; index += 1) { if (before[index] === after[index]) continue; if (before[index] !== undefined) rows.push(`- ${String(index + 1).padStart(4, " ")} ${before[index]}`); if (after[index] !== undefined) rows.push(`+ ${String(index + 1).padStart(4, " ")} ${after[index]}`); }
    return rows.join("\n") || "No source changes.";
  }, [baseline, codeDraft]);

  return <section className="liveWorkstation" aria-label="Live frontend workstation">
    <header className="workstationToolbar"><nav className="workingTabs" aria-label="Workstation views">{(["code", "diff", "logs", "media", "devtools", "frontend"] as Tab[]).map((item) => <button key={item} type="button" className={tab === item ? "active" : ""} onClick={() => setTab(item)}>{label(item)}</button>)}</nav><div className="sourceToolbar">{toolbar}<span className="routeValue" title={route}>Route <b>{route}</b></span><button type="button" className="refresh" onClick={() => setFrameKey((value) => value + 1)}>Refresh</button></div></header>
    ...
    <style jsx>{`.liveWorkstation{height:100%;min-height:0;display:grid;grid-template-rows:40px minmax(0,1fr);overflow:hidden;background:#020617;color:#e5e7eb}.workstationToolbar{min-width:0;display:flex;align-items:center;justify-content:space-between;gap:8px;padding:0 8px;border-bottom:1px solid rgba(45,212,191,.28);background:#020617;overflow:hidden}.workingTabs,.sourceToolbar{min-width:0;display:flex;align-items:center;gap:4px;overflow:auto;white-space:nowrap}.sourceToolbar{justify-content:flex-end;flex:1}.workstationToolbar button{height:28px;border:1px solid transparent;border-radius:8px;background:transparent;color:#6ee7b7;padding:0 10px;font-size:10px;font-weight:900;cursor:pointer;white-space:nowrap}.workstationToolbar button:hover{background:rgba(15,118,110,.18)}.workstationToolbar button.active{border-color:rgba(45,212,191,.55);background:rgba(6,78,59,.64);color:#99f6e4}.workstationToolbar .refresh{border-color:rgba(148,163,184,.24);color:#fff}.routeValue{max-width:180px;overflow:hidden;text-overflow:ellipsis;color:#6ee7b7;font-size:9px;text-transform:uppercase}.routeValue b{color:#fff;text-transform:none;font-size:10px}.workstationBody{position:relative;min-width:0;min-height:0;overflow:hidden;background:#020617}.workstationBody iframe{display:block;width:100%;height:100%;border:0;background:#fff}.workstationBody iframe.hidden{position:absolute;width:1px;height:1px;opacity:0;pointer-events:none}.workstationBody iframe.visible{position:relative}.diff{height:100%;overflow:auto;margin:0;padding:12px;color:#dbeafe;font:11px/17px ui-monospace,SFMono-Regular,Consolas,monospace;white-space:pre-wrap;box-sizing:border-box}.patchCue{position:absolute;z-index:50;top:10px;left:50%;transform:translateX(-50%);max-width:70%;padding:7px 12px;border:1px solid rgba(45,212,191,.55);border-radius:999px;background:rgba(2,6,23,.92);box-shadow:0 12px 30px rgba(0,0,0,.4);color:#99f6e4;font-size:10px;font-weight:900;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;pointer-events:none}.empty{height:100%;display:grid;place-content:center;padding:20px;text-align:center;color:#94a3b8}.empty.overlay{position:absolute;inset:0;background:#020617}.proofList{height:100%;overflow:auto;padding:10px;box-sizing:border-box;display:grid;align-content:start;gap:6px}.proofList p{margin:0;border-left:3px solid #64748b;background:#0f172a;padding:7px 9px;font-size:10px;line-height:1.4}.proofList p span{display:inline-block;width:76px;color:#94a3b8}.proofList p.success{border-left-color:#22c55e}.proofList p.warning{border-left-color:#f59e0b}.proofList p.error{border-left-color:#ef4444}@media(max-width:1100px){.workstationToolbar{align-items:stretch;height:auto;min-height:40px;flex-wrap:wrap;padding:5px 8px}.sourceToolbar{justify-content:flex-start}.routeValue{display:none}}`}</style>
  </section>;
}
