...
export default function VisualSelectionPatchPanel() {
  ...
  async function generatePatch(approve = false) {
    ...
    try {
      ...
      setPlan(data);
      window.dispatchEvent(new CustomEvent("streams-builder-summary-event", { detail: { phase: approve ? "visual-edit-commit" : "visual-edit-plan", message: approve ? `Visual edit committed: ${data.commits?.[0]?.commitSha || "pending"}` : `Visual edit plan ready with ${data.result?.resolved?.confidence || 0}% confidence across ${(data.indexedFiles || []).length || 1} indexed files.` } }));
    } catch (err) {
