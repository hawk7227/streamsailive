...
export default function VisualEditingWorkstation({ stationLabel, route, filePath, repo, branch, content, onContentChange, onProof, onChat }: Props) {
  ...
  useEffect(() => {
    ...
    // Intentionally do not depend on onProof; parent passes a new callback after each log update.
  ...
  function savePatch() {
    const patch = [content || "", "", "/* Streams Visual Editor Patch", `repo: ${repo}`, `branch: ${branch}`, `route: ${sourceRoute}`, `file: ${filePath}`, `selector: ${selected?.selector || ""}`, `layer: ${selected?.label || ""}`, `text: ${selected?.text || ""}`, `mode: ${viewMode}`, "*/"].join("\n");
  ...
  function updateText(nextText: string) {
    if (!selected) return;
    const note = `\n/* edit ${selected.label}: ${nextText} */`;
    onContentChange(`${content || ""}${note}`);
    record("edit-layer-text", selected.label, "pending");
  }
    ...
    <section className="visualEditor">
      ...
      <main className={`canvas ${viewMode}`}>
        ...
        <section className={viewMode === "mobile" ? "phoneFrame" : "desktopFrame"}>
          ...
          {viewMode === "editor" || viewMode === "advanced" ? <div className="editOverlay">{layers.map((layer) => <button type="button" key={layer.id} className={selected?.id === layer.id ? "active" : ""} onClick={() => selectLayer(layer)}>{layer.label}</button>)}</div> : null}
        </section>
      </main>
