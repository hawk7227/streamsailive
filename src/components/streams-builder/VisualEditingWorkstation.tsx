...
function removeFirstMatch(content: string, event: EditEvent) {
  const candidates = [event.original, event.text, event.src, stripOrigin(event.src), stripOrigin(event.original)].filter(Boolean) as string[];
  for (const candidate of candidates) {
    if (candidate && content.includes(candidate)) return content.replace(candidate, "");
  }
  return content;
}
...
export default function VisualEditingWorkstation({ stationLabel, route, filePath, repo, branch, content, onContentChange, onProof, onChat }: Props) {
  ...
  useEffect(() => {
    function record(payload: EditEvent) {
      setSelected(payload);
    ...
    function onMessage(event: MessageEvent) {
      ...
      if (data.type === "streams-editable-select") {
        setSelected(payload);
        onChat(`Selected ${payload.kind || "text"} on original page: ${payload.text || payload.src || ""}`);
      }
      if (data.type === "streams-editable-input") setSelected(payload);
      if (data.type === "streams-editable-commit") {
        record(payload);
        const nextContent = replaceFirst(draftContent || "", payload.original, payload.text);
        if (nextContent !== draftContent) applyDraft(nextContent, `Edited text: ${payload.original || ""} → ${payload.text || ""}`);
      }
      if (data.type === "streams-editable-image-replace") {
        record(payload);
        onProof(`Image replacement saved to draft event log: ${payload.replacementName || payload.src || "image"}`);
        setPatchState("not_generated");
      }
      if (data.type === "streams-editable-remove" || data.type === "streams-editable-delete") {
        record(payload);
        const nextContent = removeFirstMatch(draftContent || "", payload);
        applyDraft(nextContent, `Removed ${payload.kind || "element"} from editable preview/source draft.`);
      }
      if (data.type === "streams-editable-style" || data.type === "streams-editable-transform-start") {
        record(payload);
        onProof(`Tracked ${payload.kind || "element"} transform/style: ${payload.width || "?"}×${payload.height || "?"}`);
        setPatchState("not_generated");
      }
    }
  ...
  function saveDraft() {
    ...
    const nextCheckpointId = `checkpoint-${Date.now()}`;
    const payload = {
      id: nextDraftId,
    ...
    setPatchState("not_generated");
    onProof(`Draft saved in Streams library: ${nextDraftId}`);
  }
  ...
  async function generatePatch() {
    ...
    try {
      ...
      if (!checkpointId) setCheckpointId(activeCheckpoint);
      const result = await callLinePatches({ repo, branch, filePath, content: draftContent || content || "", push: false, sourceTruthId, checkpointId: activeCheckpoint });
      setPatchState("generated");
      onProof(`Patch generated, not pushed. Changed lines: ${result.preview?.changedLineCount ?? "unknown"}`);
    } catch (error) {
      setPatchState("failed");
      onProof(`Patch generation failed: ${error instanceof Error ? error.message : "unknown error"}`);
    } finally {
      setSaving(false);
    }
  }
  ...
  async function pushToGitHub() {
    if (!ready || patchState !== "generated") return;
    setSaving(true);
    try {
      const activeCheckpoint = checkpointId || `checkpoint-${Date.now()}`;
      ...
      onProof(`Pushed visual editor draft to GitHub${sha ? `: ${sha}` : "."}`);
      setFrameKey((value) => value + 1);
    } catch (error) {
      setPatchState("failed");
      onProof(`GitHub push failed: ${error instanceof Error ? error.message : "unknown error"}`);
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <section className="visualEditor">
      <header className="top">
        <div><b>VISUAL EDITOR</b><span>{stationLabel} · Save Draft first, Generate Patch, then Push to GitHub after review</span></div>
      ...
      <details className="editorDrawer" open={drawerOpen || viewMode === "advanced"} onToggle={(event) => setDrawerOpen(event.currentTarget.open)}><summary>Advanced edit log / source controls</summary><section className="drawerGrid"><section className="patchBox"><b>Selected</b><p>kind: {selected?.kind || "none"}</p><p>selector: {selected?.selector || "none"}</p><p>original: {selected?.original || "none"}</p><p>current: {selected?.text || selected?.src || "none"}</p><p>file: {filePath || "none"}</p></section><section className="patchBox"><b>Recent edits</b>{edits.length ? edits.slice(-8).map((edit, index) => <p key={`${edit.id}-${index}`}>{edit.kind || "text"}: {edit.original || edit.src || "item"} → {edit.text || edit.replacementName || edit.inlineStyle || "changed"}</p>) : <p>No edits yet.</p>}</section></section></details>
