      ...
      <main className={`body ${viewMode}`}>
        {viewMode !== "preview" ? (
          <section className="codePane">
            <div className="paneTitle"><b>Pulled source code</b><span>{isDirty ? "Edited draft, not pushed" : "Matches pulled SHA"}</span></div>
            <textarea value={codeDraft} onChange={(event) => setCodeDraft(event.target.value)} spellCheck="false" />
            <div className="fileInfo">{activeFile.path || "No file"} · {codeDraft.length} bytes · SHA {activeFile.sha || "missing"}</div>
          </section>
        ) : null}
