...
export default function ActualBuilderSessionChat({ connection, onConnectionChange }: Props) {
  ...
  return (
    <section className="actualBuilderChat" aria-label="Originating StreamsAI conversation">
      ...
      <form onSubmit={send}><textarea value={prompt} onChange={(event) => setPrompt(event.currentTarget.value)} placeholder="Ask anything" disabled={running || !sessionId} /><button type="submit" disabled={running || !prompt.trim()}>↑</button></form>
