...
(function(){
  ...
  function cssPath(el){var parts=[];while(el&&el.nodeType===1&&parts.length<8){var part=el.tagName.toLowerCase();if(el.id){part+="#"+el.id;parts.unshift(part);break}var cls=(el.className&&String(el.className).trim().split(/\s+/).slice(0,2).join("."))||"";if(cls)part+="."+cls;parts.unshift(part);el=el.parentElement}return parts.join(" > ")}
  function closestFingerprint(el){return el.closest('[data-streams-source-file],[data-streams-component],[data-streams-symbol],[data-streams-item-key],[data-streams-operation-target]')||el}
  function selectionFor(el){var fp=closestFingerprint(el);var img=el.tagName==='IMG'?el:el.querySelector&&el.querySelector('img');var link=el.closest&&el.closest('a[href]');var rect=el.getBoundingClientRect();return{event:'streams:visual-selection',selectionId:'sel_'+Date.now(),tag:el.tagName.toLowerCase(),text:(el.innerText||el.textContent||'').replace(/\s+/g,' ').trim().slice(0,600),parentText:(fp.innerText||fp.textContent||'').replace(/\s+/g,' ').trim().slice(0,1200),href:(link&&link.getAttribute('href'))||'',imageSrc:(img&&img.getAttribute('src'))||'',selector:cssPath(el),sourceFile:fp.getAttribute('data-streams-source-file')||'',component:fp.getAttribute('data-streams-component')||'',symbol:fp.getAttribute('data-streams-symbol')||'',itemKey:fp.getAttribute('data-streams-item-key')||'',operationTarget:fp.getAttribute('data-streams-operation-target')||'',box:{x:rect.x,y:rect.y,width:rect.width,height:rect.height},previewId:(window.__STREAMS_PREVIEW_CONTEXT__||{}).previewId||''}}
  document.addEventListener("mouseover",function(e){var el=e.target;if(!el||!el.style)return;var old=document.querySelector('[data-streams-hovering="1"]');if(old){old.style.outline=old.getAttribute('data-streams-old-outline')||'';old.removeAttribute('data-streams-hovering')}el.setAttribute('data-streams-old-outline',el.style.outline||'');el.setAttribute('data-streams-hovering','1');el.style.outline='2px solid #37e5ff';el.style.outlineOffset='2px'},true);
  document.addEventListener("click",function(e){if(!e.altKey&&!e.metaKey&&!e.ctrlKey)return; e.preventDefault();e.stopPropagation();var detail=selectionFor(e.target);window.parent&&window.parent.postMessage(detail,'*')},true);
