...
function settingStatusLabel(setting: SettingItem, busy: boolean) {
  ...
  if (setting.type === "danger") return "Protected";
  if (setting.type === "button") return String(setting.defaultValue || "Open");
  if (setting.status === "active") return "Saved";
  return "Default";
}

function renderValuePreview(setting: SettingItem, value: unknown) {
  if (setting.type === "toggle") return Boolean(value) ? "On" : "Off";
  if (setting.type === "button" || setting.type === "danger") return setting.actionLabel || "Open";
  const text = String(value ?? setting.defaultValue ?? "").trim();
  return text || "Not set";
}
...
export default function StreamsSettingsPanel({ initialCategory = "general" }: Props) {
  ...
  const loadSettings = useCallback(async () => {
    ...
    try {
      ...
      setError(safeMessage(err instanceof Error ? err.message : "Settings are temporarily unavailable."));
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    <main className={styles.settingsShell}>
        ...
        <nav className={styles.tabList}>
          {STREAMS_SETTINGS_TABS.map((tab) => (
            <Link
              ...
              href={TAB_ROUTES[tab.id] || "/account"}
              onClick={(event) => {
      ...
      <section className={styles.main}>
        <header className={styles.header}>
          <div>
            <p className={styles.eyebrow}>Streams settings</p>
            <h1>{activeMeta.title}</h1>
            <p>{activeMeta.description}</p>
          </div>
          <button type="button" className={styles.refreshButton} onClick={() => void loadSettings()} disabled={loading}>
        ...
        {activeTab === "apps" ? (
          <div className={styles.infoCard}>
            ...
            <p>Apps can provide context, search connected sources, or perform external actions. Read access, write access, sends, deletes, billing actions, and deploy actions should remain approval-gated.</p>
          </div>
        ) : null}
        ...
        {activeTab === "data-controls" ? (
          <div className={styles.infoCard}>
            ...
            <p>Data controls separate chat history, memory, connected-source access, temporary chats, exports, and product-improvement preference instead of hiding everything behind one switch.</p>
          </div>
        ) : null}

        {activeGroups.map((group) => {
          const groupSettings = group.keys
            .map((key) => activeSettings.find((setting) => setting.key === key))
            .filter((setting): setting is SettingItem => Boolean(setting));
          if (!groupSettings.length) return null;

          return (
            <section className={styles.settingGroup} key={group.title}>
                  ...
                  return (
                    ...
                    >
                      <div className={styles.settingText}>
                        ...
                        <div className={styles.saveState}>{settingStatusLabel(setting, busy)} · {preview}</div>
                      </div>
                      ...
                      {setting.type === "text" ? (
                        <input
                          ...
                          value={String(value || "")}
                          placeholder={String(setting.defaultValue || setting.label)}
                          disabled={busy}
