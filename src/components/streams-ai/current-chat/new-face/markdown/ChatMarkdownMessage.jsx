...
function scrollStreamingMessage() {
  ...
  window.requestAnimationFrame(() => {
    const node = [document.querySelector(".startChatSurface"), document.querySelector(".chatScroll"), document.querySelector(".splitChatScroll")].filter(Boolean)[0];
    if (!node) return;
...
function currentSessionId() {
  ...
  return parts[0] === "streams-ai" ? parts[1] || "" : "";
}

async function postMessageAction(payload) {
...
function ResponseActions({ content }) {
  ...
  useEffect(() => {
    ...
    const close = (event) => {
      if (event.key === "Escape" || (event.type === "pointerdown" && wrapRef.current && !wrapRef.current.contains(event.target))) setMenuOpen(false);
    };
  ...
  const rate = (value) => {
    setFeedback(value);
    setStatus(value === "feedback_up" ? "Thanks for the feedback" : "Feedback recorded");
    log(value);
    window.setTimeout(() => setStatus(""), 1400);
  };

  const share = async () => {
    if (navigator.share) await navigator.share({ title: "Streams response", text: content });
    else await navigator.clipboard.writeText(content);
    setStatus(navigator.share ? "Shared" : "Copied for sharing");
    log("shared");
    window.setTimeout(() => setStatus(""), 1400);
  };
  ...
  const regenerate = async () => {
    ...
    try {
      const key = `regenerate:${messageMeta.id || content.slice(0, 48)}`;
      await postMessageAction({ action: "regenerate", sessionId, messageId: messageMeta.id, content, idempotencyKey: key });
  ...
  const branch = async () => {
    ...
    try {
      const data = await postMessageAction({ action: "branch", sessionId, messageId: messageMeta.id, content });
      window.location.assign(data.href || `/streams-ai/${data.sessionId}`);
    } catch (error) {
      setStatus(error instanceof Error ? error.message : "Branch failed");
      setBusy(false);
    }
  };
  ...
  const readAloud = () => {
    ...
    utterance.onstart = () => { setStatus("Reading aloud…"); log("read_aloud_started"); };
    utterance.onend = () => { setStatus(""); log("read_aloud_completed"); };
    utterance.onerror = () => setStatus("Read aloud stopped");
    window.speechSynthesis.speak(utterance);
  };

  return (
    <div className="chatResponseFooter" ref={wrapRef}>
      <div className="chatResponseActions" aria-label="Response actions">
        ...
        <div className="chatResponseMore">
          <button type="button" onClick={() => { setMenuOpen((value) => !value); log("more_menu_opened"); }} aria-label="More response actions" aria-expanded={menuOpen}>•••</button>
          {menuOpen ? (
            <div className="chatResponseMenu" role="menu">
              <div className="chatResponseTimestamp">Today, {formatTimestamp(messageMeta.createdAt)}</div>
              <button type="button" onClick={branch}>↗ Branch in new chat</button>
...
function ChatMarkdownMessage({ content }) {
  ...
  useEffect(() => {
    ...
    const step = () => {
      ...
      if (index < chunks.length) {
        const delay = Math.max(STREAM_PACING.minimumChunkDelayMs, Math.min(STREAM_PACING.maximumChunkDelayMs, next.length * STREAM_PACING.millisecondsPerCharacter));
  ...
  return (
    <div className="chatMarkdown">
      ...
      {fullMarkdown ? <ResponseActions content={fullMarkdown} /> : null}
    </div>
  );
}
