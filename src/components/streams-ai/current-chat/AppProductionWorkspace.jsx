...
export default function AppProductionWorkspace() {
  ...
  return <main className={`appStart phase-${phase}`} aria-label="Streams App Project">
    ...
    <style jsx global>{`
      .appStart{height:100dvh;min-height:690px;overflow:hidden;position:relative;background:radial-gradient(circle at 50% -8%,rgba(0,229,255,.15),transparent 30%),linear-gradient(180deg,#02090f,#02070c 70%,#031018);color:#edfefe;font-family:Inter,ui-sans-serif,system-ui}.appStart *{box-sizing:border-box}.appStart button,.appStart textarea{font:inherit}.appCircuit{position:absolute;top:74px;width:220px;height:110px;opacity:.28;background:repeating-linear-gradient(90deg,transparent 0 32px,rgba(8,221,236,.45) 33px 34px,transparent 35px 64px);mask-image:linear-gradient(to bottom,black,transparent)}.appCircuitLeft{left:0}.appCircuitRight{right:0;transform:scaleX(-1)}
      ...
      .appComposer,.appFlow{width:min(1190px,94vw);justify-self:center}.appComposer{min-height:116px;border:1px solid rgba(8,221,236,.9);border-radius:24px;padding:13px 18px;background:linear-gradient(135deg,rgba(5,22,33,.96),rgba(3,14,24,.92));box-shadow:0 18px 70px rgba(0,205,226,.12);display:grid;gap:8px}.appComposer:focus-within{box-shadow:0 0 0 1px rgba(8,221,236,.28),0 0 45px rgba(0,221,236,.22)}.appPrompt{display:grid;grid-template-columns:auto 1fr;gap:12px;align-items:start}.appPrompt>svg{width:30px;color:#08ddec;margin-top:5px}.appComposer textarea{width:100%;min-height:52px;max-height:170px;resize:none;overflow:auto;border:0;outline:0;background:transparent;color:#effcff;font-size:17px;line-height:1.45;white-space:pre-wrap}.appComposer textarea::placeholder{color:#aebac3}.appActions{display:flex;align-items:center;gap:12px}.appActions>button:first-child{min-height:38px;padding:0 14px;border:1px solid rgba(0,221,236,.55);border-radius:8px;background:rgba(0,221,236,.04);color:#15e9f5;display:flex;align-items:center;gap:8px;font-weight:700}.appActions span{margin-left:auto;color:#91a2ad;font-size:11px}.appSubmit{width:50px;height:50px;border:0;border-radius:50%;display:grid;place-items:center;background:linear-gradient(145deg,#1af0f7,#08bfe3);color:#00212a}.appSubmit:disabled{opacity:.38}.appFiles{display:flex;gap:8px;overflow:auto}.appFiles>div{position:relative;width:96px;height:64px;border:1px solid #164459;border-radius:10px;overflow:hidden;display:grid;place-items:center;background:#03121c}.appFiles img{width:100%;height:100%;object-fit:cover}.appFiles small{position:absolute;left:3px;right:3px;bottom:3px;background:#03131ddd;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;padding:3px;font-size:8px}.appFiles button{position:absolute;right:3px;top:3px;width:21px;height:21px;border:0;border-radius:50%;background:#001017;color:white}
      .appFlow{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));align-items:center;gap:10px;padding:2px 0 4px}.appFlow>div{display:grid;grid-template-columns:34px minmax(0,1fr) 18px;align-items:center;gap:8px;min-width:0;color:#506a78}.appFlow>div>span{width:34px;height:34px;border:1px solid #245365;border-radius:50%;display:grid;place-items:center;font-size:11px}.appFlow>div>b{font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.appFlow>div>svg{width:17px}.appFlow>div:last-child{grid-template-columns:34px minmax(0,1fr)}.appFlow>div.active{color:#08ddec}.appFlow>div.active>span{border-color:#08ddec;background:rgba(0,221,236,.08)}.appFlow>div.active>span svg{width:15px}
      .appQuick{width:100%;display:flex;justify-content:center;gap:10px;overflow:auto;scrollbar-width:none}.appQuick button{flex:0 0 auto;min-height:40px;padding:0 18px;border:1px solid rgba(93,143,164,.32);border-radius:999px;background:rgba(4,17,27,.55);color:#dff9fc;display:flex;align-items:center;gap:8px}.appQuick button:hover,.appQuick button.active{border-color:#08ddec;color:#08ddec;background:rgba(0,221,236,.09)}.appQuick svg{width:17px}.appGrid{min-height:0;display:grid;grid-template-columns:repeat(3,minmax(0,1fr));grid-template-rows:repeat(3,minmax(0,1fr));gap:10px}.appCard{position:relative;min-width:0;min-height:0;overflow:hidden;border:0;border-radius:13px;background:#06131c;color:white;text-align:left;padding:0;cursor:pointer;box-shadow:0 10px 30px rgba(0,0,0,.24)}.appVisual{position:absolute;inset:0 38% 0 0;overflow:hidden}.appVisual img{width:100%;height:100%;object-fit:cover;transition:transform .3s ease}.appVisualShade{position:absolute;inset:0;background:linear-gradient(90deg,transparent 50%,rgba(3,12,19,.9))}.appCopy{position:absolute;inset:0 12px 0 63%;display:grid;align-content:center;gap:7px}.appCopy strong{font-size:15px}.appCopy small{color:#b7c6ce;font-size:10px;line-height:1.43}.appCopy b{display:flex;align-items:center;justify-content:space-between;color:#11e0ed;font-size:10px}.appCard:hover,.appCard.active{transform:translateY(-2px);box-shadow:0 18px 44px rgba(0,214,235,.13)}.appCard:hover img,.appCard.active img{transform:scale(1.04)}.appCard.active{outline:1px solid #08ddec}
      .blueprintStage{grid-row:3/6;min-height:0;display:grid;grid-template-rows:auto minmax(0,1fr) auto;gap:14px;width:min(1320px,100%);justify-self:center}.blueprintIntro{display:flex;align-items:center;justify-content:center;gap:14px}.blueprintIntro>span{width:48px;height:48px;border-radius:50%;display:grid;place-items:center;background:rgba(0,221,236,.1);color:#08ddec}.blueprintIntro small{color:#08ddec;font-weight:800}.blueprintIntro h2{margin:2px 0 0}.blueprintIntro p{margin:3px 0 0;color:#99acb7}.blueprintLayout{min-height:0;display:grid;grid-template-columns:1.45fr 1fr;gap:14px}.blueprintPreview,.blueprintDetails article{background:rgba(7,25,37,.92);border:1px solid rgba(63,130,153,.22);border-radius:16px}.blueprintPreview{padding:16px;display:grid;grid-template-rows:auto auto minmax(0,1fr);gap:11px}.blueprintPreview header{display:flex;align-items:center;gap:10px}.blueprintPreview header>span{width:40px;height:40px;border-radius:10px;display:grid;place-items:center;background:rgba(0,221,236,.1);color:#08ddec}.blueprintPreview header h3{margin:1px 0 0}.blueprintPreview header i{margin-left:auto;color:#4ade80;font-size:10px}.previewWindow{min-height:0;border-radius:12px;overflow:hidden;background:#edf5f7;color:#17313c}.previewTop{height:34px;background:#dce9ed;display:flex;align-items:center;gap:6px;padding:0 10px}.previewTop i{width:7px;height:7px;border-radius:50%;background:#8ca7b0}.previewBody{height:calc(100% - 34px);display:grid;grid-template-columns:150px 1fr}.previewBody aside{padding:12px;background:#153744;color:#e8fbff;display:grid;align-content:start;gap:9px}.previewBody aside span{display:flex;gap:6px;font-size:9px}.previewBody section{padding:14px;display:grid;gap:14px}.metricRow{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}.metricRow b,.chartMock{padding:10px;background:white;border-radius:8px}.chartMock{display:flex;align-items:end;gap:8px}.chartMock span{flex:1;background:linear-gradient(#08ddec,#0f7e9b)}.chartMock span:nth-child(1){height:30%}.chartMock span:nth-child(2){height:48%}.chartMock span:nth-child(3){height:42%}.chartMock span:nth-child(4){height:70%}.chartMock span:nth-child(5){height:86%}.blueprintDetails{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}.blueprintDetails article{padding:14px}.blueprintDetails h4,.blueprintDetails p{display:flex;align-items:center;gap:7px}.blueprintActions{display:flex;justify-content:flex-end;gap:10px}.blueprintActions button{min-height:42px;padding:0 16px;border:1px solid rgba(96,145,162,.32);border-radius:9px;background:#06151f;color:#dff9fc}.approveButton{background:linear-gradient(135deg,#0fd6e5,#14b8a6)!important;color:#00272c!important}.spin{animation:appSpin .8s linear infinite}@keyframes appSpin{to{transform:rotate(360deg)}}
      @media(max-width:720px){.appStart{min-height:100dvh}.appInner{padding:10px 12px;gap:7px}.appHero h1{font-size:27px}.appHero>p{font-size:11px}.appComposer,.appFlow{width:100%}.appComposer{min-height:112px;border-radius:18px;padding:11px}.appComposer textarea{font-size:14px}.appActions span{display:none}.appFlow{display:flex;overflow-x:auto;gap:14px;padding-bottom:4px}.appFlow>div{flex:0 0 auto;grid-template-columns:30px auto 14px}.appFlow>div>span{width:30px;height:30px}.appQuick{justify-content:flex-start}.appQuick button{min-height:36px;padding:0 13px;font-size:11px}.appGrid{display:flex;overflow-x:auto;scroll-snap-type:x mandatory}.appCard{flex:0 0 88vw;height:100%;scroll-snap-align:start}.appVisual{inset:0 0 38%}.appVisualShade{background:linear-gradient(180deg,transparent 42%,rgba(3,12,19,.92))}.appCopy{inset:auto 14px 13px}.blueprintLayout{display:block;overflow:auto}.blueprintDetails{grid-template-columns:1fr}.blueprintActions button{flex:1}}
    `}</style>
  </main>;
}
