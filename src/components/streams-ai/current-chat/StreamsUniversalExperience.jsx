...
function ProjectCreationDialog({ open, onClose, onCreated }) {
  ...
  async function createProject(event) {
    ...
    try {
      ...
      const response = await fetch("/api/v1/projects", {
        ...
        body: JSON.stringify({
          ...
          metadata: {
            ...
            progress: 5,
            nextRecommendedAction: projectType === "Coding / Application" ? "Open the builder workspace" : "Review the generated project workspace",
      ...
      window.localStorage.setItem(EXPERIENCE_VIEW_KEY, "workspace");
  ...
  return (
    <div className="projectCreationBackdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <form className="projectCreationDialog" role="dialog" aria-modal="true" aria-labelledby="project-creation-title" onSubmit={createProject}>
        <header>
          <div>
            <strong id="project-creation-title">Create a StreamsAI project</strong>
            <span>Streams will select the right workspace automatically.</span>
          </div>
          <button type="button" onClick={onClose} aria-label="Close project creation">×</button>
        </header>
        <label>
          <span>1. What do you want to create or complete?</span>
          <textarea value={goal} onChange={(event) => setGoal(event.target.value)} autoFocus required />
        </label>
        <label>
          <span>2. Do you have files, images, notes, or references?</span>
          <textarea value={references} onChange={(event) => setReferences(event.target.value)} />
        </label>
        <label>
          <span>3. What should the finished result look like?</span>
          <textarea value={finishedResult} onChange={(event) => setFinishedResult(event.target.value)} />
        </label>
        <label>
          <span>4. Are there requirements or constraints?</span>
        ...
        <footer>
          ...
          <button type="submit" disabled={saving}>{saving ? "Creating…" : "Create and open workspace"}</button>
        </footer>
      </form>
    </div>
  );
...
export default function StreamsUniversalExperience() {
  ...
  useEffect(() => {
    ...
    const stored = window.localStorage.getItem(EXPERIENCE_VIEW_KEY);
    const initial = requested === "workspace" || requested === "chat" ? requested : stored === "workspace" ? "workspace" : "chat";
    setView(initial);
  ...
  function changeView(nextView) {
    setView(nextView);
    window.localStorage.setItem(EXPERIENCE_VIEW_KEY, nextView);
  ...
  return (
    <div className="streamsUniversalExperience" data-active-view={view}>
      {view === "chat" ? <StreamsClientShell /> : <ProjectWorkspaceShell />}
      <nav className="experienceSwitcher" aria-label="Streams experience">
        <button type="button" className={view === "chat" ? "active" : ""} onClick={() => changeView("chat")}>Chat</button>
        <button type="button" className={view === "workspace" ? "active" : ""} onClick={() => changeView("workspace")}>{activeProjectName ? `Workspace · ${activeProjectName}` : "Workspace"}</button>
      ...
      />
      <style jsx global>{`
        .streamsUniversalExperience{min-height:100svh;background:#080b18}
        .experienceSwitcher{position:fixed;top:10px;left:50%;transform:translateX(-50%);z-index:50000;display:flex;align-items:center;gap:4px;padding:4px;border:1px solid rgba(148,163,184,.3);border-radius:12px;background:rgba(4,10,24,.94);box-shadow:0 10px 30px rgba(0,0,0,.35);backdrop-filter:blur(14px)}
        .experienceSwitcher button{min-height:34px;max-width:260px;padding:0 12px;border:0;border-radius:8px;background:transparent;color:#cbd5e1;font-size:11px;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
        .experienceSwitcher button.active{background:#1d4ed8;color:#fff}.experienceSwitcher .createProjectAction{background:#065f46;color:#d1fae5}
        ...
        .projectCreationDialog footer{justify-content:flex-end}.projectCreationDialog footer button{min-height:38px;padding:0 14px;border:1px solid rgba(148,163,184,.3);border-radius:9px;background:#111827;color:#e2e8f0;font-weight:800}.projectCreationDialog footer button[type=submit]{background:#1d4ed8;border-color:#3b82f6;color:#fff}.projectCreationDialog footer button:disabled{opacity:.6}
        @media(max-width:760px){.experienceSwitcher{top:auto;bottom:max(10px,env(safe-area-inset-bottom));width:calc(100% - 20px);justify-content:center}.experienceSwitcher button{flex:1;padding:0 8px}.projectCreationBackdrop{padding:10px}.projectCreationDialog{max-height:calc(100svh - 20px);border-radius:14px;padding:14px}}
      `}</style>
    </div>
  );
}
