...
export default function StreamsUniversalExperience() {
  ...
  const isStandaloneCreative = isStandaloneVideo || isStandaloneImage || isStandaloneResearch;
  const showWorkspaceNavigation = pathname === "/streams-ai" && activeView === "chat" && !isStandaloneCreative;
  const destinationView = destination === "business-builder" ? <MarketingCampaignWorkspace onNewProject={() => setCreating(true)} /> : isStandaloneVideo ? <VideoProductionWorkspace /> : isStandaloneImage ? <ImageProductionWorkspace /> : isStandaloneResearch ? <ResearchMarketingWorkspace /> : <StreamsDestinationWorkspace destination={destination} onNewProject={() => setCreating(true)} />;
