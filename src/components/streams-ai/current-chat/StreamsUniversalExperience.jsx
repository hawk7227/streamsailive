  ...
  return (
    <div className="projectCreationBackdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
      <form className="projectCreationDialog" role="dialog" aria-modal="true" aria-labelledby="project-creation-title" onSubmit={createProject}>
        <header><div><strong id="project-creation-title">Create a StreamsAI project</strong><span>Streams will select the right project URL automatically.</span></div><button type="button" onClick={onClose} aria-label="Close project creation">×</button></header>
        ...
        <div className="detectedProjectType"><span>Detected project route</span><strong>{projectType}</strong></div>
...
export default function StreamsUniversalExperience() {
  ...
  return (
    <div className="streamsUniversalExperience" data-active-view="chat">
      <StreamsClientShell />
      <nav className="experienceSwitcher" aria-label="Streams experience">
        <button type="button" className="active">Chat</button>
        <button type="button" onClick={() => router.push("/streams-ai/streams-builder/workspace")}>{activeProjectName ? `Workspace · ${activeProjectName}` : "Workspace"}</button>
        <button type="button" onClick={() => router.push("/streams-ai/streams-builder/research")}>Research</button>
        <button type="button" onClick={() => router.push("/streams-ai/streams-builder/gen-video")}>Gen Video</button>
      ...
      <style jsx global>{`
        ...
        .experienceSwitcher button{min-height:34px;max-width:260px;padding:0 12px;border:0;border-radius:8px;background:transparent;color:#cbd5e1;font-size:11px;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.experienceSwitcher button.active{background:#1d4ed8;color:#fff}.experienceSwitcher .createProjectAction{background:#065f46;color:#d1fae5}
        ...
        @media(max-width:760px){.experienceSwitcher{top:auto;bottom:max(10px,env(safe-area-inset-bottom));width:calc(100% - 20px);justify-content:center;overflow:auto}.experienceSwitcher button{flex:0 0 auto;padding:0 8px}.projectCreationBackdrop{padding:10px}.projectCreationDialog{max-height:calc(100svh - 20px);border-radius:14px;padding:14px}}
      `}</style>
    </div>
  );
}
