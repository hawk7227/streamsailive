...
const NAV_GROUPS = [
  { title: "MAIN", items: [["home", "AI Operator Home"], ["portfolio", "Brand Portfolio"], ["projects", "Projects & Concepts"]] },
  { title: "BUILD", items: [["business", "Business Builder"], ["revenue", "Revenue / Income Potential"], ["visuals", "Visual Concepts / 3D Artifacts"], ["website", "Website Builder"], ["app", "App Builder"], ["launch", "Preview + Launch"]] },
  { title: "CREATE", items: [["studio", "Creator Studio"], ["image", "AI Image Studio"], ["video", "AI Video Studio"], ["voice", "AI Voice Studio"], ["captions", "Captions & Subtitles"], ["content", "Content Generator"], ["ideas", "Ideas & Topics"], ["turn-you", "Turn This Into You"], ["calendar", "Content Calendar"], ["social", "Social Research"], ["growth-feed", "Growth Feed / Marketing Feed"], ["assets", "Assets Library"]] },
  { title: "GROW", items: [["checklist", "Launch Checklist"], ["growth", "Growth Dashboard"], ["notifications", "Notifications"]] },
  { title: "ACCOUNT", items: [["billing", "Billing / Credits / Hosting"], ["notification-settings", "Notification Settings"], ["settings", "Settings"]] },
];
...
function DesktopSidebar({ activeSection, setActiveSection, onNewChat }) {
  return (
    <aside className="streamsOpSidebar" aria-label="Streams AI navigation">
      <div className="streamsOpBrandBlock"><div className="streamsOpLogo">S</div><div><strong>STREAMS AI</strong><span>Visual Operator</span></div></div>
    ...
    <section className="streamsOpChatPanel" aria-label="AI Command Chat">
      <div className="streamsOpChatHeader">
        <div>
          ...
          <h1>{activeProject ? activeProject.title : "Ask anything"}</h1>
          <p>{activeProject ? "Project memory is isolated to this selected project only." : "General mode · no project memory loaded · choose Portfolio to open a project."}</p>
        </div>
      ...
      <div className="streamsOpChatScroll">
        {!hasMessages ? (
          <div className="streamsOpChatEmpty">
            <h2>{activeProject ? "Tell Streams AI what to build next." : "What do you want to work on?"}</h2>
            <p>The current broad assistant ability stays available for writing, coding, architecture, UI/UX, marketing, troubleshooting, research, documentation, and problem-solving.</p>
          </div>
...
function ProjectPicker({ sessions, activeProject, onSelectProject, onStartCleanProject }) {
  ...
  return (
    <section className="streamsOpModuleScreen">
      <span className="streamsOpEyebrow">BRAND PORTFOLIO</span>
      <h2>Open the project you want to work on</h2>
      ...
      <div className="streamsOpPickerActions"><button type="button" className="streamsOpPrimary noMargin" onClick={onStartCleanProject}>Start clean project</button><button type="button" className="streamsOpSecondary" onClick={() => onSelectProject(null)}>General assistant mode</button></div>
      {recent.length ? (
...
function InlineBuildPanel({ activeProject, inlineOpen, onClose }) {
  ...
  return (
    <aside className="streamsOpInlinePanel" aria-label="Inline Build panel">
      ...
      <div className="streamsOpPreviewHero"><div className="streamsOpPreviewGlow" /><span>{mode} Mode</span><h3>{activeProject ? "Ready to render this project visually" : "Select a project to open inline build"}</h3><p>{mode === "Marketing" ? "Marketing Preview is ready for real sample promos, draft ads, and Turn This Into You flows when generation is wired." : mode === "Artifact" ? "The 3D artifact belongs inside this preview as an ambient visual mode, not a large standalone block." : mode === "Progress" ? "Build progress should show real saved jobs and real generated outputs only." : "Concept Preview is the live visual surface for the selected brand, project, app, website, or creator idea."}</p></div>
      ...
      <div className="streamsOpInlineActions"><button type="button">Save Concept</button><button type="button">View Revenue Plan</button><button type="button">Turn This Into You</button></div>
      <p className="streamsOpTruthNote">No fake generation: live videos, promos, domains, SMS, and launch actions must connect to real stored outputs before they appear as complete.</p>
    </aside>
  );
}
...
function HonestModuleScreen({ id, activeProject }) {
  ...
  return <section className="streamsOpModuleScreen"><span className="streamsOpEyebrow">{activeProject ? "PROJECT SCOPED" : "GENERAL MODE"}</span><h2>{title}</h2><p>{body}</p><div className="streamsOpModuleCards"><div><b>Status</b><span>Shell ready</span></div><div><b>Rule</b><span>No fake outputs</span></div><div><b>Memory</b><span>{activeProject ? "Selected project only" : "No project loaded"}</span></div></div></section>;
}
