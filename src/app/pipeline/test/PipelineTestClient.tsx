...
type QueueItem = { id: string; type: "image" | "video" | "script"; status: QueueStatus; provider: string; model?: string | null; prompt: string; conceptId: string | null; startedAt: number; completedAt: number | null; outputUrl: string | null; externalId: string | null; mode: string; costEstimate: number; elapsedSeconds: number; error: string | null; };
...
type OutputTarget = "center" | "left_phone" | "right_phone" | "shelf";
type AssetApprovalState = "draft" | "processing" | "qc_failed" | "approved" | "stitched";
...
interface ClipJobPlan {
  id: string;
  chapterId: string;
  sceneId: string;
  type: "scene_script" | "scene_image" | "scene_video" | "scene_qc" | "scene_stitch";
  retryCount: number;
  status: QueueStatus | "queued";
}

interface VideoPlan {
  intent: GenerationIntentAction;
  ...
  stitchStatus: QueueStatus | "queued";
}
...
interface RealismProfile {
  profileName: string;
  hardNegativeList: string[];
  hardPositiveList: string[];
...
function inferDurationMode(minutes: number): DurationMode {
...
function buildLongVideoPlan(minutes: number, intent: GenerationIntentAction): VideoPlan {
  const durationMode = inferDurationMode(minutes);
  const chapterCount = durationMode === "feature" ? Math.max(6, Math.ceil(minutes / 12)) : durationMode === "long" ? Math.max(3, Math.ceil(minutes / 10)) : 1;
  const chapters: ChapterPlan[] = Array.from({ length: chapterCount }, (_, chapterIndex) => {
    const chapterId = `chapter-${chapterIndex + 1}`;
    const targetMinutes = Math.max(1, Math.round(minutes / chapterCount));
    const sceneCount = durationMode === "feature" ? 8 : durationMode === "long" ? 5 : 3;
    const scenes: ScenePlan[] = Array.from({ length: sceneCount }, (_, sceneIndex) => ({
      id: `${chapterId}-scene-${sceneIndex + 1}`,
      chapterId,
      title: `Scene ${sceneIndex + 1}`,
      targetSeconds: Math.max(8, Math.round((targetMinutes * 60) / sceneCount)),
      clipCount: durationMode === "feature" ? 6 : durationMode === "long" ? 4 : 2,
      status: sceneIndex === 0 && chapterIndex === 0 ? "processing" : "pending",
    }));
    return {
      id: chapterId,
      title: `Chapter ${chapterIndex + 1}`,
      targetMinutes,
      status: chapterIndex === 0 ? "processing" : "pending",
      scenes,
    };
  });
  ...
  return {
    ...
    clipCount: chapters.reduce((sum, chapter) => sum + chapter.scenes.reduce((sceneTotal, scene) => sceneTotal + scene.clipCount, 0), 0),
    chapters,
    currentChapterId: chapters[0]?.id ?? null,
    currentSceneId: chapters[0]?.scenes[0]?.id ?? null,
    stitchStatus: "pending",
  };
}

function buildGenerationPlanFromChat(input: string): { action: GenerationIntentAction; requestedMinutes: number } {
  const hourMatch = input.match(/(\d+(?:\.\d+)?)\s*(hour|hr)/i);
  const minuteMatch = input.match(/(\d+(?:\.\d+)?)\s*(minute|min)/i);
  const requestedMinutes = hourMatch ? Math.round(parseFloat(hourMatch[1]) * 60) : minuteMatch ? Math.round(parseFloat(minuteMatch[1])) : 1;
  const wantsVideo = /video|scene|clip|film|movie/i.test(input);
  const action: GenerationIntentAction = wantsVideo
    ? requestedMinutes > 15 ? "PLAN_LONG_VIDEO" : "PLAN_VIDEO"
    : "PLAN_IMAGE";
  return { action, requestedMinutes };
}

function compileRealismPrompt(input: string, realismProfile: RealismProfile): string {
  const negative = realismProfile.hardNegativeList.join(", ");
  const positive = realismProfile.hardPositiveList.join(", ");
  return `${input}. Keep this grounded in ordinary real life. Prioritise ${positive}. Strictly avoid ${negative}.`;
}

function AmbientLayer({ tint = "cyan", intensity = 1 }: { tint?: "cyan" | "violet" | "gold"; intensity?: number }) {
  const tintMap = {
    cyan: ["rgba(103,232,249,0.22)", "rgba(56,189,248,0.08)"],
    violet: ["rgba(168,85,247,0.18)", "rgba(129,140,248,0.08)"],
    gold: ["rgba(251,191,36,0.16)", "rgba(249,115,22,0.08)"],
  } as const;
  const [a, b] = tintMap[tint];
  return (
    <div
      aria-hidden
      style={{
...
- Would look like a real unposed moment — not advertising, not staged`,
  ...
  const [generationIntent, setGenerationIntent] = useState<GenerationIntentAction>("PLAN_IMAGE");
  const [outputTarget, setOutputTarget] = useState<OutputTarget>("center");
  const [videoPlan, setVideoPlan] = useState<VideoPlan>(() => buildLongVideoPlan(1, "PLAN_VIDEO"));
  const [continuityProfile, setContinuityProfile] = useState<ContinuityProfile>({
    subjectIdentity: "Primary subject",
    environment: "Lived-in real environment",
    lighting: "Natural uneven daylight",
    cameraStyle: "Phone camera realism",
    motionBehavior: "Subtle grounded motion",
  });
  const [realismProfile, setRealismProfile] = useState<RealismProfile>({
    profileName: "Everyday Realism",
    hardNegativeList: ["cinematic lighting", "beauty retouch", "perfect symmetry", "editorial pose", "overly polished skin", "ad-style composition"],
    hardPositiveList: ["natural skin texture", "ordinary real-life framing", "soft imperfect light", "lived-in background", "subtle asymmetry"],
    rejectionReasons: ["plastic skin", "fake lighting", "AI-sharp detail", "staged composition"],
  });
  const [clipJobs, setClipJobs] = useState<ClipJobPlan[]>([]);
  const [assetApprovalStatusMap, setAssetApprovalStatusMap] = useState<Record<string, AssetApprovalState>>({});
  ...
  React.useEffect(() => {
    const nextMap: Record<string, AssetApprovalState> = {};
    mediaShelfItems.forEach((item) => {
      const raw = String(item.status ?? "").toLowerCase();
      nextMap[item.id] = raw.includes("fail") ? "qc_failed" : raw.includes("process") || raw.includes("pending") ? "processing" : raw.includes("stitched") ? "stitched" : "approved";
    });
    setAssetApprovalStatusMap(nextMap);
  }, [mediaShelfItems]);

  React.useEffect(() => {
    const input = `${storyTitle} ${storyText}`.trim();
    const planned = buildGenerationPlanFromChat(input);
    setGenerationIntent(planned.action);
    const nextPlan = buildLongVideoPlan(planned.requestedMinutes, planned.action);
    setVideoPlan(nextPlan);
    const jobs: ClipJobPlan[] = nextPlan.chapters.flatMap((chapter) =>
      chapter.scenes.flatMap((scene) => ([
        { id: `${scene.id}-script`, chapterId: chapter.id, sceneId: scene.id, type: "scene_script", retryCount: 0, status: scene.id === nextPlan.currentSceneId ? "processing" : "pending" },
        { id: `${scene.id}-image`, chapterId: chapter.id, sceneId: scene.id, type: "scene_image", retryCount: 0, status: "pending" },
        { id: `${scene.id}-video`, chapterId: chapter.id, sceneId: scene.id, type: "scene_video", retryCount: 0, status: "pending" },
        { id: `${scene.id}-qc`, chapterId: chapter.id, sceneId: scene.id, type: "scene_qc", retryCount: 0, status: "pending" },
        { id: `${scene.id}-stitch`, chapterId: chapter.id, sceneId: scene.id, type: "scene_stitch", retryCount: 0, status: "pending" },
      ]))
    );
    setClipJobs(jobs);
  ...
  React.useEffect(() => {
    setStepPrompts((prev) => ({
      ...prev,
      imagery: compileRealismPrompt(prev.imagery || "Generate one realism-first still image", realismProfile),
      i2v: `${prev.i2v} Maintain subject continuity: ${continuityProfile.subjectIdentity}; environment: ${continuityProfile.environment}; lighting: ${continuityProfile.lighting}; motion: ${continuityProfile.motionBehavior}.`,
      qa: `Reject outputs for: ${realismProfile.rejectionReasons.join(", ")}. Preserve continuity for ${continuityProfile.subjectIdentity} in ${continuityProfile.environment}.`,
    }));
  }, [realismProfile, continuityProfile]);
  ...
  async function generateImage(conceptId: string, overridePrompt?: string) {
    const rawPrompt = (overridePrompt && overridePrompt.trim()) || stepPrompts.imagery;
    const prompt = compileRealismPrompt(rawPrompt, realismProfile);
  ...
  async function generateVideo(conceptId: string) {
    const planned = buildGenerationPlanFromChat(`${storyTitle} ${storyText}`);
    const nextPlan = buildLongVideoPlan(planned.requestedMinutes, planned.action);
    setGenerationIntent(planned.action);
    setVideoPlan(nextPlan);
    setClipJobs(nextPlan.chapters.flatMap((chapter) => chapter.scenes.map((scene) => ({ id: `${scene.id}-video`, chapterId: chapter.id, sceneId: scene.id, type: "scene_video" as const, retryCount: 0, status: scene.id === nextPlan.currentSceneId ? "processing" : "pending" }))));
    ...
    const prompt = compileRealismPrompt(stepPrompts.i2v + (concept ? ` Concept: ${concept.headline}.` : ""), realismProfile);
    ...
    <>
      ...
      <div style={{
        ...
        <iframe
          ...
          title="STREAMS Chat"
          style={{ flex: 1, border: "none", display: "block", background: "#0b1220", minWidth: 0 }}
        />
        ...
        >
          <div style={{ position: "absolute", top: 0, bottom: 0, left: "50%", transform: "translateX(-50%)", width: 2, background: chatHandleActive ? "rgba(34,211,238,0.65)" : "rgba(255,255,255,0.12)" }} />
        </div>
      </div>
    ...
    <>
      ...
      {showConflictModal && (
        <div style={{position:"fixed",inset:0,zIndex:2000,background:"rgba(0,0,0,0.8)",display:"flex",alignItems:"center",justifyContent:"center",backdropFilter:"blur(4px)"}}
          ...
          <div style={{width:"100%",maxWidth:620,maxHeight:"85vh",background:"#141d31",border:`1px solid ${hardConflicts.length>0?"rgba(239,68,68,0.4)":"rgba(245,158,11,0.4)"}`,borderRadius:16,display:"flex",flexDirection:"column",boxShadow:"0 18px 60px rgba(0,0,0,0.6)",overflow:"hidden"}}>
            {/* Header */}
            <div style={{padding:"18px 22px 14px",background:hardConflicts.length>0?"rgba(239,68,68,0.08)":"rgba(245,158,11,0.08)",borderBottom:"1px solid rgba(255,255,255,0.07)"}}>
              ...
              {parsedGuidance&&<div style={{marginTop:8,fontSize:10,color:"rgba(255,255,255,0.35)",background:"rgba(255,255,255,0.055)",border:"1px solid rgba(255,255,255,0.07)",borderRadius:6,padding:"5px 10px"}}>{parsedGuidance.summary}</div>}
            </div>
            {/* Bulk resolve */}
            <div style={{padding:"8px 22px",background:"rgba(255,255,255,0.02)",borderBottom:"1px solid rgba(255,255,255,0.09)",display:"flex",alignItems:"center",gap:8}}>
                    ...
                    <div style={{padding:"8px 12px 10px",background:"rgba(255,255,255,0.015)"}}>
                      ...
                      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
                        {[{l:"Frontend",v:c.frontendValue,active:!resolved||fWon,color:"#67e8f9",bg:"rgba(103,232,249,0.08)",bd:"rgba(103,232,249,0.2)"},{l:"Guidance",v:c.guidanceValue,active:!resolved||!fWon,color:"#c4b5fd",bg:"rgba(168,85,247,0.08)",bd:"rgba(168,85,247,0.2)"}].map(({l,v,active,color,bg,bd})=>(
                          <div key={l} style={{padding:"7px 10px",borderRadius:7,border:`1px solid ${active?bd:"rgba(255,255,255,0.09)"}`,background:active?bg:"transparent",transition:"all 200ms"}}>
          ...
          {!isEmbed && <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 0, position: "relative" }}>
              ...
              {presetDropOpen && namedPresets.length > 0 && (
                <div style={{ position: "absolute", top: "calc(100% + 4px)", left: 0, zIndex: 200, background: "#172133", border: "1px solid rgba(103,232,249,0.2)", borderRadius: 10, minWidth: 220, boxShadow: "0 10px 30px rgba(0,0,0,0.4)", overflow: "hidden" }}>
                  {namedPresets.map(p => (
                    <button key={p.id} onClick={() => loadNamedPreset(p)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", width: "100%", padding: "9px 14px", background: "transparent", border: "none", borderBottom: "1px solid rgba(255,255,255,0.09)", color: "#f1f5f9", fontSize: 12, cursor: "pointer", textAlign: "left" }}>
                      <span>{p.name}</span>
            ...
            <select value={pipelineMode} onChange={e => setPipelineMode(e.target.value as "manual" | "auto")}
              style={{ background: "rgba(255,255,255,0.09)", border: "1px solid rgba(255,255,255,0.14)", color: "#fff", borderRadius: 10, padding: "8px 12px", fontSize: 13, cursor: "pointer" }}>
  ...
  {(["kling-v3", "veo-3.1"] as const).map(p => (
    <button key={p} onClick={() => setVideoProvider(p)}
      ...
      {p === "kling-v3" ? "Kling v3" : "Veo 3.1"}
            ...
            <button onClick={() => setQueueTrayOpen(p => !p)}
              style={{ background: "rgba(255,255,255,0.09)", border: "1px solid rgba(255,255,255,0.14)", color: "#94a3b8", borderRadius: 10, padding: "8px 12px", fontSize: 13, cursor: "pointer" }}>
              ⬡ Queue
            </button>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isEmbed ? `${stepConfigOpen ? "320px" : "48px"} minmax(0,1fr)` : `${leftOpen ? "clamp(220px,18vw,300px)" : "48px"} ${stepConfigOpen ? "320px" : "48px"} minmax(0,1fr)`, gap: 14, marginBottom: 14, transition: "grid-template-columns 200ms ease", minHeight: 720, alignItems: "stretch" }}>
            ...
            <div style={P({ padding: 14, display: "flex", flexDirection: "column", gap: 10 })}>
                ...
                {[
                  ...
                  <div key={f.key} style={{ display:"flex", alignItems:"center", gap:6, marginBottom:6 }}>
                    ...
                    <input value={csFields[f.key]??""} onChange={e=>setCsFields(p=>({...p,[f.key]:e.target.value}))}
                      style={{ flex:1, background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.12)", borderRadius:6, color:"#e2e8f0", fontSize:13, padding:"7px 10px", outline:"none" }} />
                  </div>
                ))}
              </div>
                  ...
                  {[
                    ...
                    <div key={f.key} style={{ flex:1 }}>
                      ...
                      <select value={csFields[f.key]??""} onChange={e=>setCsFields(p=>({...p,[f.key]:e.target.value}))}
                        style={{ width:"100%", background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.12)", borderRadius:6, color:"#e2e8f0", fontSize:12, padding:"6px 8px", outline:"none" }}>
                        <option value="">—</option>
              ...
              <div>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:8 }}>
                  ...
                  <select value={csRealism.mode} onChange={e=>setCsRealism(p=>({...p,mode:e.target.value as "STRICT"|"STANDARD"|"SOFT"|"RAW"}))}
                    style={{ flex:1, background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.12)", borderRadius:6, color:"#e2e8f0", fontSize:13, padding:"6px 8px", outline:"none" }}>
              ...
              <div>
                ...
                <textarea value={csFields.csFinalPrompt??""} onChange={e=>setCsFields(p=>({...p,csFinalPrompt:e.target.value}))}
                  ...
                  rows={3}
                  style={{ width:"100%", background:"rgba(255,255,255,0.065)", border:"1px solid rgba(255,255,255,0.12)", borderRadius:8, color:"#e2e8f0", fontSize:12, padding:"8px 10px", outline:"none", resize:"vertical", lineHeight:1.5, boxSizing:"border-box" }} />
              ...
              <div style={{ display:"flex", gap:8 }}>
                ...
                <button onClick={()=>guidanceInputRef.current?.click()}
                  style={{ padding:"10px 12px", borderRadius:10, border:"1px solid rgba(255,255,255,0.1)", background:"rgba(255,255,255,0.065)", color:"#94a3b8", fontSize:12, cursor:"pointer" }}>
              ...
              <div>
                ...
                <textarea value={csFields.csPipelinePrompt??""} onChange={e=>setCsFields(p=>({...p,csPipelinePrompt:e.target.value}))}
                  ...
                  rows={3}
                  style={{ width:"100%", background:"rgba(255,255,255,0.065)", border:`1px solid ${csFields.csPipelinePrompt?"rgba(103,232,249,0.3)":"rgba(255,255,255,0.12)"}`, borderRadius:8, color:"#e2e8f0", fontSize:11, padding:"8px 10px", outline:"none", resize:"vertical", lineHeight:1.5, boxSizing:"border-box" }} />
              </div>
              </>)}
            </div>
            ...
            <div style={P({ padding: 14, display: "flex", flexDirection: "column", gap: 6 })}>
              ...
              {steps.map(step => (
                <div key={step.id} onClick={() => selectStep(step.id)}
                  ...
                  style={{ display: "flex", alignItems: "center", gap: 12, padding: "13px 16px", borderRadius: 12,
                    background: step.state === "running" ? "rgba(124,58,237,0.12)" : liveStepId === step.id ? "rgba(124,58,237,0.08)" : selectedStepId === step.id ? "rgba(168,85,247,0.12)" : "rgba(255,255,255,0.055)",
              ...
              <button onClick={() => setSteps(p => [...p, { id: `custom-${Date.now()}`, name: "Custom Step", state: "queued", icon: "+", output: null, error: null, startedAt: null, completedAt: null }])}
                style={{ marginTop: 8, background: "rgba(255,255,255,0.055)", border: "1px dashed rgba(255,255,255,0.12)", color: "#475569", borderRadius: 10, padding: "10px 0", fontSize: 14, cursor: "pointer" }}>
                + Add Step
              </button>
            </div>
              ...
              {!stepConfigOpen ? (
                ...
                <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 12, height: "100%" }}>
                  ...
                  <textarea value={stepPrompts[selectedStepId] ?? ""}
                    ...
                    rows={8}
                    style={{ flex: 1, background: "rgba(255,255,255,0.065)", border: "1px solid rgba(255,255,255,0.10)", color: "#e2e8f0", borderRadius: 10, padding: 10, fontSize: 12, resize: "none", lineHeight: 1.55 }} />
                  <button onClick={async () => {
            ...
            <div style={P({ display: "flex", flexDirection: "column", height: "100%", position: "relative" })}>
              ...
              <div style={{
                ...
                <div
                  ...
                  <div style={{ position: "absolute", top: 0, bottom: 0, left: "50%", transform: "translateX(-50%)", width: 2, background: epHandleActive ? "rgba(68,195,166,0.65)" : "rgba(255,255,255,0.12)" }} />
                </div>
                {/* EditorPro iframe */}
              ...
              {/* Tabs */}
              <div style={{ display: "flex", borderBottom: "1px solid rgba(255,255,255,0.12)", padding: "0 16px" }}>
                    ...
                    return (
                      <div style={{ display: "flex", flexDirection: "column", height: "100%", alignItems: "center" }}>
                        ...
                        <div style={{
                          ...
                          <div style={{ position: "absolute", left: -btnW, top: Math.round(80 * S), width: btnW, height: actionH, background: "linear-gradient(90deg, #1a1a1c, #2a2a2e)", borderRadius: `${btnW}px 0 0 ${btnW}px`, boxShadow: "-1px 0 0 rgba(255,255,255,0.12)" }} />
                          {/* Volume up */}
                          <div style={{ position: "absolute", left: -btnW, top: Math.round(120 * S), width: btnW, height: volH, background: "linear-gradient(90deg, #1a1a1c, #2a2a2e)", borderRadius: `${btnW}px 0 0 ${btnW}px`, boxShadow: "-1px 0 0 rgba(255,255,255,0.12)" }} />
                          {/* Volume down */}
                          <div style={{ position: "absolute", left: -btnW, top: Math.round(165 * S), width: btnW, height: volH, background: "linear-gradient(90deg, #1a1a1c, #2a2a2e)", borderRadius: `${btnW}px 0 0 ${btnW}px`, boxShadow: "-1px 0 0 rgba(255,255,255,0.12)" }} />
                          {/* Power button — right side */}
                          <div style={{ position: "absolute", right: -btnW, top: Math.round(130 * S), width: btnW, height: powerH, background: "linear-gradient(270deg, #1a1a1c, #2a2a2e)", borderRadius: `0 ${btnW}px ${btnW}px 0`, boxShadow: "1px 0 0 rgba(255,255,255,0.12)" }} />
                          ...
                          <div style={{
                            ...
                            <div style={{
                              ...
                              <div style={{
                                ...
                                boxShadow: "0 0 0 1px rgba(255,255,255,0.055), inset 0 0 4px rgba(0,0,0,0.8)",
                              }} />
                            </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", flex: 1, minHeight: 0, height: "100%" }}>
                      ...
                      {showDestPicker && pendingResult && (
                        <div style={{ position: "absolute", inset: 0, zIndex: 500, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", borderRadius: 12 }}>
                          <div style={{ background: "#18243a", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 14, padding: "22px 24px", minWidth: 280 }}>
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                              {([["iphone1","iPhone 15 Pro Max #1","◱"],["iphone2","iPhone 15 Pro Max #2","◱"],["desktop","Desktop / Wide View","▭"]] as [PreviewDestination,string,string][]).map(([dest, label, icon]) => (
                                ...
                                }} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderRadius: 9, border: "1px solid rgba(255,255,255,0.1)", background: previewDestination===dest?"rgba(103,232,249,0.1)":"rgba(255,255,255,0.055)", color: "#f1f5f9", fontSize: 12, cursor: "pointer", textAlign: "left" }}>
                      ...
                      <div style={{ padding: "4px 12px", borderTop: "1px solid rgba(255,255,255,0.09)", display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
                        <button type="button" onClick={() => setBatchModalOpen(true)}
                          style={{ fontSize: 10, color: "rgba(255,255,255,0.45)", background: "rgba(255,255,255,0.065)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 6, padding: "3px 8px", cursor: "pointer" }}>
                          ⊞ Batch preview
                        </button>
                        <button type="button" onClick={() => setWireframeMode(w => !w)}
                          style={{ fontSize: 10, color: wireframeMode ? "#a78bfa" : "rgba(255,255,255,0.35)", background: wireframeMode ? "rgba(167,139,250,0.1)" : "rgba(255,255,255,0.055)", border: `1px solid ${wireframeMode ? "rgba(167,139,250,0.3)" : "rgba(255,255,255,0.12)"}`, borderRadius: 6, padding: "3px 8px", cursor: "pointer" }}>
                          ⬜ {wireframeMode ? "Wireframe ON" : "Wireframe"}
                        </button>
                        <button type="button" onClick={() => setShowSafeZone(s => !s)}
                          style={{ fontSize: 10, color: showSafeZone ? "#fb923c" : "rgba(255,255,255,0.35)", background: showSafeZone ? "rgba(251,146,60,0.1)" : "rgba(255,255,255,0.055)", border: `1px solid ${showSafeZone ? "rgba(251,146,60,0.3)" : "rgba(255,255,255,0.12)"}`, borderRadius: 6, padding: "3px 8px", cursor: "pointer" }}>
                        ...
                        <button type="button" title="Export iPhone #1 as PNG" onClick={async () => {
                          ...
                          style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", background: "rgba(255,255,255,0.055)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 6, padding: "3px 8px", cursor: "pointer" }}>
                          ↓ Export #1
                        </button>
                      ...
                      {/* Playback bar */}
                      <div style={{ padding: "10px 16px 14px", borderTop: "1px solid rgba(255,255,255,0.09)", display: "flex", alignItems: "center", justifyContent: "center", gap: 20, flexShrink: 0 }}>
                        {/* Rewind */}
                        <button onClick={() => { [playbackRef1, playbackRef2].forEach(r => { if (r.current) { r.current.currentTime = 0; } }); }}
                          style={{ background: "rgba(255,255,255,0.09)", border: "1px solid rgba(255,255,255,0.1)", color: "#94a3b8", width: 38, height: 38, borderRadius: "50%", fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>⏮</button>
                        {/* Play/Pause */}
                        <button onClick={() => {
                          ...
                          style={{ background: playbackPlaying ? "rgba(103,232,249,0.15)" : "rgba(255,255,255,0.12)", border: "1px solid "+(playbackPlaying?"rgba(103,232,249,0.4)":"rgba(255,255,255,0.1)"), color: playbackPlaying?"#67e8f9":"#94a3b8", width: 38, height: 38, borderRadius: "50%", fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                          {playbackPlaying ? "⏸" : "▷"}
                        </button>
                        {/* Stop */}
                  ...
                  <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: 12, maxWidth: 480 }}>
                    ...
                    {[
                      ...
                      <button key={item.label} onClick={item.action} disabled={!item.enabled}
                        style={{ display: "flex", alignItems: "center", gap: 12, background: "rgba(255,255,255,0.065)", border: "1px solid rgba(255,255,255,0.10)", color: item.enabled ? "#94a3b8" : "#334155", borderRadius: 10, padding: "12px 16px", fontSize: 13, cursor: item.enabled ? "pointer" : "not-allowed", textAlign: "left" }}>
                ...
                {workspaceTab === "publish" && (
                  <div style={{ width: "100%", maxWidth: 480, display: "flex", flexDirection: "column", gap: 12 }}>
                    ...
                    {["Meta Ads Manager", "Google Ads", "TikTok Ads", "Instagram"].map(platform => (
                      <div key={platform} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(255,255,255,0.065)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 10, padding: "12px 16px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 0.65fr", gap: 14, marginBottom: 14, alignItems: "stretch" }}>
            ...
            <div style={P({ padding: 0, display: "flex", flexDirection: "column", minHeight: 320 })}>
              ...
              {/* Filter tabs */}
              <div style={{ display: "flex", gap: 0, padding: "0 14px", borderBottom: "1px solid rgba(255,255,255,0.09)" }}>
                ...
                {mediaShelfItems
                  ...
                  .map(item => (
                    <div key={item.id}
                      ...
                      style={{ position: "relative", borderRadius: 8, overflow: "hidden", aspectRatio: "16/9", background: "#0a0f1e", cursor: "pointer", border: "1px solid rgba(255,255,255,0.09)" }}>
            ...
            <div style={P({ padding: 0, display: "flex", flexDirection: "column", minHeight: 320 })}>
              ...
              <div style={{ width: "100%", aspectRatio: "4/3", background: "#10192d", overflow: "hidden", borderBottom: "1px solid rgba(255,255,255,0.12)", flexShrink: 0, position: "relative" }}>
              ...
              <div style={{ padding: "10px 12px", borderBottom: "1px solid rgba(255,255,255,0.09)" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
                  <button onClick={() => void generateImage(selectedConceptId)}
                    style={{ padding: "7px 0", fontSize: 10, fontWeight: 600, borderRadius: 8, border: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.065)", color: "#94a3b8", cursor: "pointer" }}>
                    🖼 Image
                  </button>
                  <button onClick={() => { if (!storyBible) { void buildStoryBibleRecord().then(() => void generateVideo(selectedConceptId)); } else { void generateVideo(selectedConceptId); } }}
                    style={{ padding: "7px 0", fontSize: 10, fontWeight: 600, borderRadius: 8, border: `1px solid ${storyBible ? "rgba(255,255,255,0.1)" : "rgba(251,191,36,0.3)"}`, background: storyBible ? "rgba(255,255,255,0.065)" : "rgba(251,191,36,0.06)", color: storyBible ? "#94a3b8" : "#fbbf24", cursor: "pointer" }}>
                ...
                {storySongVoiceTab === "story" && (
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    ...
                    <button onClick={() => { setStepConfigOpen(true); setSelectedStepId("strategy"); }}
                      style={{ padding: "7px 10px", fontSize: 10, borderRadius: 8, border: "1px solid rgba(255,255,255,0.12)", background: "rgba(255,255,255,0.055)", color: "#64748b", cursor: "pointer", textAlign: "left" }}>
                      ⚙ Video Settings
                    </button>
          ...
          <div style={{ background: "linear-gradient(180deg, rgba(15,24,44,0.96), rgba(10,18,34,0.96))", border: "1px solid rgba(148,163,184,0.18)", borderRadius: 18, marginBottom: 14, boxShadow: "0 18px 46px rgba(2,6,23,.18)" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 16px", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
              ...
              {(["16:9", "9:16"] as const).map(r => (
                <button key={r} onClick={() => setConceptGridRatio(r)}
                  style={{ padding: "4px 12px", fontSize: 10, fontWeight: 700, borderRadius: 8,
                    background: conceptGridRatio === r ? "rgba(103,232,249,0.12)" : "rgba(255,255,255,0.065)",
                    border: `1px solid ${conceptGridRatio === r ? "rgba(103,232,249,0.35)" : "rgba(255,255,255,0.12)"}`,
              ...
              <button onClick={() => gatedGeneration(() => Promise.all(concepts.map(cv => generateImage(cv.variantId))))}
                style={{ padding: "5px 12px", fontSize: 10, fontWeight: 700, borderRadius: 8, border: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.065)", color: "#94a3b8", cursor: "pointer" }}>
                ▶ Run All Images
              </button>
              <button onClick={() => gatedGeneration(() => Promise.all(concepts.map(cv => generateVideo(cv.variantId))))}
                style={{ padding: "5px 12px", fontSize: 10, fontWeight: 700, borderRadius: 8, border: `1px solid ${storyBible ? "rgba(255,255,255,0.1)" : "rgba(251,191,36,0.3)"}`, background: storyBible ? "rgba(255,255,255,0.065)" : "rgba(251,191,36,0.06)", color: storyBible ? "#94a3b8" : "#fbbf24", cursor: "pointer" }}>
                  ...
                  <div style={{ position: "absolute", inset: 0, overflow: "hidden" }}>
                    ...
                    <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, display: "flex", gap: 4, padding: "6px 8px", background: "linear-gradient(to top, rgba(0,0,0,0.85), transparent)" }}>
                      ...
                      <button onClick={() => gatedGeneration(() => generateImage(cid))} disabled={isActive}
                        style={{ flex: 1, padding: "5px 0", fontSize: 9, fontWeight: 700, borderRadius: 6, border: "1px solid rgba(255,255,255,0.1)", background: "rgba(255,255,255,0.09)", color: "#94a3b8", cursor: "pointer" }}>
                        Image
                      </button>
                      <button onClick={() => storyBible ? gatedGeneration(() => generateVideo(cid)) : void buildStoryBibleRecord()} disabled={isActive}
                        style={{ flex: 1, padding: "5px 0", fontSize: 9, fontWeight: 700, borderRadius: 6, border: `1px solid ${storyBible ? "rgba(255,255,255,0.1)" : "rgba(251,191,36,0.3)"}`, background: storyBible ? "rgba(255,255,255,0.09)" : "rgba(251,191,36,0.08)", color: storyBible ? "#94a3b8" : "#fbbf24", cursor: "pointer" }}>
                        {storyBible ? "Video" : "⚠ Video"}
                      </button>
                      <button onClick={() => { setConceptOutputs(p => ({ ...p, [cid]: { image: null, video: null, script: null, status: "idle", error: null } })); }}
                        style={{ padding: "5px 7px", fontSize: 9, borderRadius: 6, border: "1px solid rgba(255,255,255,0.12)", background: "rgba(255,255,255,0.065)", color: "#475569", cursor: "pointer" }}>
                        Reset
                      </button>
                    </div>
                  </div>
          ...
          {queueTrayOpen && (
            <div style={{ position: "fixed", bottom: 24, right: 24, width: 360, maxHeight: 480, background: "rgba(8,12,33,0.97)", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 16, boxShadow: "0 18px 60px rgba(0,0,0,.3)", display: "flex", flexDirection: "column", zIndex: 1000 }}>
              <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.12)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              {/* Filter tabs */}
              <div style={{ display: "flex", padding: "8px 12px", gap: 4, borderBottom: "1px solid rgba(255,255,255,0.09)", overflowX: "auto" }}>
      ...
      {diagResult && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}
          ...
          <div style={{ background: "#172233", border: "1px solid rgba(255,255,255,0.12)", borderRadius: 16, padding: 28, maxWidth: 640, width: "100%", maxHeight: "80vh", overflowY: "auto" }}
