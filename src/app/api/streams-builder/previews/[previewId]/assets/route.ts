...
export async function GET(request: NextRequest, { params }: { params: Promise<{ previewId: string }> }) {
  try {
    ...
    const { data, error } = await db().from("streams_ai_preview_assets").select("*, asset:streams_ai_assets(*)").eq("tenant_id", scope.tenantId).eq("user_id", scope.userId).eq("preview_id", previewId).order("created_at", { ascending: false });
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ previewId: string }> }) {
  try {
    ...
    }, { onConflict: "tenant_id,user_id,preview_id,asset_id" }).select("*, asset:streams_ai_assets(*)").single();
    if (error) throw error;
    return streamsAIJson({ ok: true, previewAsset: data }, 201);
  } catch (error) { return streamsAIError(error); }
}
