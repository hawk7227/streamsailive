...
(() => {
  ...
  function keepScroll(fn) {
    ...
    const top = doc ? doc.scrollTop : y;
    const left = doc ? doc.scrollLeft : x;
    try { fn(); } finally {
      window.scrollTo(x, y);
      if (doc) { doc.scrollTop = top; doc.scrollLeft = left; }
  ...
  function payloadFor(el) {
    ...
    return { id: el.dataset.streamsEditableId, kind, selector: cssPath(el), text: kind === 'text' ? textOf(el) : textOf(el).slice(0, 160), original: el.dataset.streamsOriginalText || el.dataset.streamsOriginalSrc || '', src: kind === 'image' ? (el.tagName === 'IMG' ? absoluteSrc(el.getAttribute('src')) : absoluteSrc(bgUrl(el))) : '', inlineStyle: el.getAttribute('style') || '', width: Math.round(el.getBoundingClientRect().width), height: Math.round(el.getBoundingClientRect().height), transform: style.transform === 'none' ? '' : style.transform };
  }
  ...
  function selectElement(el) { keepScroll(() => { if (selected) selected.removeAttribute('data-streams-selected'); selected = el; el.dataset.streamsSelected = 'true'; moveToolbar(el); post('streams-editable-select', payloadFor(el)); }); }
  ...
  function markEditable() {
    ...
    textNodes.forEach((el, index) => { if (!isEditableTextElement(el)) return; el.dataset.streamsEditable = 'true'; el.dataset.streamsEditableKind = 'text'; el.dataset.streamsEditableId = 'text-' + index; el.dataset.streamsOriginalText = textOf(el); el.setAttribute('contenteditable', 'true'); el.setAttribute('spellcheck', 'false'); makeResizableAndMovable(el); el.addEventListener('mousedown', event => event.preventDefault()); el.addEventListener('click', event => { event.preventDefault(); event.stopPropagation(); focusNoScroll(el); selectElement(el); }); el.addEventListener('input', () => post('streams-editable-input', payloadFor(el))); el.addEventListener('blur', () => post('streams-editable-commit', payloadFor(el))); });
    const imageNodes = Array.from(document.body.querySelectorAll('img, picture img, [style*="background-image"]'));
    imageNodes.forEach((el, index) => { if (!isEditableImageElement(el)) return; el.dataset.streamsEditable = 'true'; el.dataset.streamsEditableKind = 'image'; el.dataset.streamsEditableId = 'image-' + index; el.dataset.streamsOriginalSrc = el.tagName === 'IMG' ? absoluteSrc(el.getAttribute('src')) : absoluteSrc(bgUrl(el)); makeResizableAndMovable(el); el.addEventListener('click', event => { event.preventDefault(); event.stopPropagation(); selectElement(el); }); });
