...
(() => {
  ...
  function visibleRect(el) { const rect = el.getBoundingClientRect(); return rect.width > 8 && rect.height > 8 ? rect : null; }
  ...
  function isEditableContainerElement(el) {
    ...
    const hasMedia = Boolean(el.querySelector('img,video,[style*="background-image"]'));
  ...
  function payloadFor(el) {
    const kind = el.dataset.streamsEditableKind || (el.tagName === 'IMG' ? 'image' : el.tagName === 'VIDEO' ? 'video' : 'text');
    const style = window.getComputedStyle(el);
    return { id: el.dataset.streamsEditableId, kind, selector: cssPath(el), text: kind === 'text' ? textOf(el) : textOf(el).slice(0, 160), original: el.dataset.streamsOriginalText || el.dataset.streamsOriginalSrc || '', src: (kind === 'image' || kind === 'video') ? mediaSrc(el) : '', inlineStyle: el.getAttribute('style') || '', width: Math.round(el.getBoundingClientRect().width), height: Math.round(el.getBoundingClientRect().height), transform: style.transform === 'none' ? '' : style.transform };
  }
  function createToolbar() {
    ...
    toolbar.addEventListener('click', event => { const button = event.target.closest('button'); if (!button || !selected) return; const act = button.dataset.act; if (act === 'replace') replaceImage(selected); if (act === 'remove') removeSelected(selected); if (act === 'rotate') rotateSelected(selected); if (act === 'front') { selected.style.zIndex = String((Number(selected.style.zIndex) || 10) + 1); post('streams-editable-style', payloadFor(selected)); } if (act === 'done') deselect(); });
    return toolbar;
  }
  function moveToolbar(el) { const bar = createToolbar(); const rect = el.getBoundingClientRect(); bar.style.left = Math.max(8, rect.left + window.scrollX) + 'px'; bar.style.top = Math.max(8, rect.top + window.scrollY - 42) + 'px'; bar.style.display = 'flex'; const replace = bar.querySelector('[data-act="replace"]'); if (replace) replace.style.display = el.dataset.streamsEditableKind === 'image' ? 'inline-flex' : 'none'; }
  ...
  function markEditable() {
    ...
    textNodes.forEach((el, index) => { if (!isEditableTextElement(el)) return; el.dataset.streamsEditable = 'true'; el.dataset.streamsEditableKind = 'text'; el.dataset.streamsEditableId = 'text-' + index; el.dataset.streamsOriginalText = textOf(el); el.setAttribute('contenteditable', 'true'); el.setAttribute('spellcheck', 'false'); makeResizableAndMovable(el); el.addEventListener('click', event => { event.stopPropagation(); keepPreviewScroll(() => { try { el.focus({ preventScroll: true }); } catch { el.focus(); } selectElement(el); }); }); el.addEventListener('input', () => post('streams-editable-input', payloadFor(el))); el.addEventListener('blur', () => post('streams-editable-commit', payloadFor(el))); });
    const imageNodes = Array.from(document.body.querySelectorAll('img, picture img, video, [style*="background-image"]'));
    imageNodes.forEach((el, index) => { if (!isEditableImageElement(el)) return; const isVideo = el.tagName === 'VIDEO'; el.dataset.streamsEditable = 'true'; el.dataset.streamsEditableKind = isVideo ? 'video' : 'image'; el.dataset.streamsEditableId = 'image-' + index; el.dataset.streamsOriginalSrc = mediaSrc(el); makeResizableAndMovable(el); el.addEventListener('click', event => { event.preventDefault(); event.stopPropagation(); selectElement(el); }); });
    const containerNodes = Array.from(document.body.querySelectorAll('main,section,article,header,footer,div'));
    containerNodes.forEach((el, index) => { if (!isEditableContainerElement(el)) return; el.dataset.streamsEditable = 'true'; el.dataset.streamsEditableKind = 'container'; el.dataset.streamsEditableId = 'container-' + index; el.dataset.streamsOriginalText = textOf(el).slice(0, 180); makeResizableAndMovable(el); el.addEventListener('dblclick', event => { event.preventDefault(); event.stopPropagation(); selectElement(el); }); });
  }
  const style = document.createElement('style');
  style.textContent = '[data-streams-editable="true"]{outline:1px solid transparent;outline-offset:2px}[data-streams-editable-kind="text"]{cursor:text}[data-streams-editable-kind="image"],[data-streams-editable-kind="video"],[data-streams-editable-kind="container"]{cursor:pointer}[data-streams-editable="true"]:hover,[data-streams-selected="true"]{outline:2px solid #f97316!important;box-shadow:0 0 0 1px rgba(249,115,22,.45),0 0 18px rgba(249,115,22,.24)!important}[data-streams-editable="true"]:focus{outline:2px solid #f97316!important;background:rgba(249,115,22,.08)!important}.streams-edit-toolbar{position:absolute;z-index:2147483647;display:none;gap:6px;align-items:center;padding:6px;border:1px solid rgba(249,115,22,.7);border-radius:10px;background:rgba(2,6,23,.96);box-shadow:0 12px 32px rgba(0,0,0,.35)}.streams-edit-toolbar button{height:28px;border:1px solid rgba(255,255,255,.18);border-radius:8px;background:#7c3aed;color:#fff;font:700 11px system-ui;padding:0 8px;cursor:pointer}';
...
export async function GET(request: NextRequest) {
  ...
  if (!contentType.includes("text/html")) { const body = await response.arrayBuffer(); return new NextResponse(body, { status: response.status, headers: { "content-type": contentType || "application/octet-stream" } }); }
