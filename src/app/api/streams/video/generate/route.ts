...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  const { prompt, duration, aspectRatio } = validated.body;
  ...
  const { error: insertError } = await admin.from("generation_log").insert({
    ...
    fal_endpoint:    FAL_ENDPOINTS.KLING_V3_T2V,
    input_params:    { prompt, duration, aspectRatio },
    fal_status:      "pending",
  ...
  // 6. Submit to fal
  // Kling v3 T2V input schema — confirmed from fal.ts source + audit
  ...
  const submitResult = await falSubmit(FAL_ENDPOINTS.KLING_V3_T2V, falInput);
  ...
  console.log(JSON.stringify({
    ...
    generationId,
    workspaceId,
    endpoint: FAL_ENDPOINTS.KLING_V3_T2V,
    duration,
    aspectRatio,
  }));
